...
#include "ShaderQueueSuit_CommonUt.h"
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.6", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObject = LoadUtil::loadFromFile("../models/1306_konglong.obj", device, memoryProperties);
    objObjectUt = LoadUtil::loadFromFileUt("../models/1306_konglong.obj", device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    ...
    LightManager::setLightPosition(0, 10, 100);
...
void VulkanDemoApp::flushUniformBuffer() {// 将当前帧相关数据送入一致变量缓冲
    ...
    result = vkMapMemory(device, sqsUt->memUniformBuf, 0, sqsUt->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsUt->bufferByteCount);
    vkUnmapMemory(device, sqsUt->memUniformBuf);
}
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::texNames.size() / 2; i++) {
        sqsUt->writes[0].dstSet = sqsUt->descSet[i];
        sqsUt->writes[1].dstSet = sqsUt->descSet[i];
        sqsUt->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 2 + 0]]);
        sqsUt->writes[2].dstSet = sqsUt->descSet[i];
        sqsUt->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 2 + 1]]);
        vkUpdateDescriptorSets(device, 3, sqsUt->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    MatrixState3D::translate(20, 0, -60);
    MatrixState3D::rotate(yAngle, 0, 1, 0);
    MatrixState3D::rotate(xAngle, 1, 0, 0);
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1306_konglong.bntex")]));
    ...
    MatrixState3D::rotate(yAngle, 0, 1, 0);
    MatrixState3D::rotate(xAngle, 1, 0, 0);
    objObjectUt->drawSelf(cmdBuffer, sqsUt->pipelineLayout, sqsUt->pipeline, &(sqsUt->descSet[0]));
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsUt = new ShaderQueueSuit_CommonUt(&device, renderPass, memoryProperties);
}
