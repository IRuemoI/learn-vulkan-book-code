...
float VulkanDemoApp::SPAN = 1.0f;
float VulkanDemoApp::cz = 100;
float VulkanDemoApp::targetz = 0;
float VulkanDemoApp::preCZ = cz;
float VulkanDemoApp::preTargetZ = targetz;
float *VulkanDemoApp::mViewProjectionInverseMatrix = new float[16];
float *VulkanDemoApp::mPreviousProjectionMatrix;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    ...
    vkUpdateDescriptorSets(device, 3, sqsD2D->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex()//将场景绘制到2 幅纹理的方法
{
    const char *tp = "../textures/1501_";
    ...
    MatrixState3D::scale(0.1f, -0.1f, 0.1f);
    landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky(tp + (string) ("grass.bntex"))]));
...
void VulkanDemoApp::drawSceneToScreen()//通过2 幅纹理生成屏幕最终画面的的方法
{
...
void VulkanDemoApp::drawTrees(int treeIndex, float transX, float transY, float transZ) {
    ...
    if (treeIndex == 1) {
        ...
        tree1->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("tree.bntex"))]));
        MatrixState3D::popMatrix();
    } else {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree2->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("tree1.bntex"))]));
        MatrixState3D::popMatrix();
    }
}
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsCT = new ShaderQueueSuit_CommonTex(&device, renderPassSelf, memoryProperties);
