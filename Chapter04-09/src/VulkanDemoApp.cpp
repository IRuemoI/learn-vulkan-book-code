...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.9", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    BeltData::genVertexData();                                                                                                        // 生成待绘制对象的顶点数据和颜色数据
    beltForDraw = new DrawableObjectCommonLight(BeltData::vdata, BeltData::dataByteCount, BeltData::vCount, device, memoryProperties);// 创建绘制用待绘制对象
}
...
void VulkanDemoApp::drawObject() {
    ...
    beltForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));
