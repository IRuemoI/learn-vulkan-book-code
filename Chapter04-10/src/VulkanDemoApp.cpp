...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createVulkanSwapChain() {
    ...
    result = vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[USED_GPU_INDEX], surface, &formatCount, nullptr);// 获取支持的格式数量
    assert(result == VK_SUCCESS);
    printf("支持的格式数量为 %d\n", formatCount);
...
void VulkanDemoApp::createDrawableObject() {
    ...
    CircleData::genVertexData();                                                                                                                                                                              // 生成待绘制对象的顶点数据和颜色数据
    cirForDraw = new DrawableObjectCommonLight(CircleData::vdata, CircleData::dataByteCount, CircleData::vCount, CircleData::idata, CircleData::indexByteCount, CircleData::iCount, device, memoryProperties);// 创建绘制用待绘制对象
}
...
void VulkanDemoApp::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.5f,
                                     1000);// 设置投影参数
}

void VulkanDemoApp::flushUniformBuffer() const// 将当前帧相关数据送入一致变量缓冲
{
...
void VulkanDemoApp::drawObject() {
    ...
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0, CircleData::iCount);//绘制正十边形
    MatrixState3D::popMatrix();                                                                                          //恢复现场

    MatrixState3D::pushMatrix();                                                                                                 //保护现场
    MatrixState3D::translate(0, -50, 0);                                                                                         //沿Y 轴负方向平移50
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0, CircleData::iCount / 2 + 1);//绘制正十边形的下半部分
    MatrixState3D::popMatrix();                                                                                                  //恢复现场
