...
void ShaderQueueSuit_CommonTex::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonTex::initDescriptorSet(VkDevice &device) {
    VkDescriptorPoolSize type_count[4];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::texNames.size() / 3;
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::texNames.size() / 3;
    type_count[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[2].descriptorCount = TextureManager::texNames.size() / 3;
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = nullptr;
    descriptor_pool.maxSets = TextureManager::texNames.size() / 3;
    ...
    for (int i = 0; i < TextureManager::texNames.size() / 3; i++) {
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNames.size() / 3;
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::texNames.size() / 3);
