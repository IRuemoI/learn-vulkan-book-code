...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 9.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    CommonTexDataObject *landObject = FileUtil::loadCommonTexData("../textures/0905_land.bntex");
    LandData *land = new LandData(landObject->width, landObject->height);
    landForDraw = new TexDrawableObject(land->vData, land->vCount * 7 * 4, land->vCount, device, memoryProperties);
}
...
void VulkanDemoApp::flushTexToDesSet() const {//将纹理等数据与描述集关联的方法
    for (int i = 0; i < 1; i++) {             //遍历所有纹理组（本案例中仅有一组）
        ...
        sqsDT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 3 + 0]]);
        sqsDT->writes[2].dstSet = sqsDT->descSet[i];
        sqsDT->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 3 + 1]]);
        sqsDT->writes[3].dstSet = sqsDT->descSet[i];                                                           //更新描述集对应的写入属性(灰度图纹理)
        sqsDT->writes[3].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 3 + 2]]);//写入属性对应的纹理信息(灰度图纹理)
        vkUpdateDescriptorSets(device, 4, sqsDT->writes, 0, nullptr);                                          //更新描述集
    }
}
