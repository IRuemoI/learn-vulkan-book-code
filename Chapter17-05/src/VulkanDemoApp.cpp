...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::creat_QueryPool() {//创建查询池的方法
    ...
    qpci.pNext = nullptr;//自定义数据的指针
    qpci.flags = 0;//标志位
    qpci.queryType = VK_QUERY_TYPE_OCCLUSION;//指定查询池的查询方式
    qpci.queryCount = queryCount;//给定查询总数
    qpci.pipelineStatistics = 0;//控制统计如何执行的附加标志(这里为空)
    VkResult result = vkCreateQueryPool(device, &qpci, nullptr, &queryPool);//创建查询池
    assert(result == VK_SUCCESS);//检查查询池创建是否成功
    printf("查询池创建成功\n");
}
void VulkanDemoApp::destroy_QueryPool() {//销毁查询池
    delete[] queryResultData;//释放查询结果数据数组
    vkDestroyQueryPool(device, queryPool, nullptr);//销毁对应的查询池
}
...
void VulkanDemoApp::createDrawableObject() {
    objObjectZDT = LoadUtil::loadFromFile("../models/1705_zdt.obj", device, memoryProperties);
    objObjectQT = LoadUtil::loadFromFile("../models/1705_qt100.obj", device, memoryProperties);
}
...
void VulkanDemoApp::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 2.0f, 500);
...
void VulkanDemoApp::drawObject() {
    ...
    vkCmdResetQueryPool(cmdBuffer, queryPool, 0, queryCount);//重置查询池  查询池中的每个查询都被标记为可用或不可用。 最初，池中的所有查询处于未定义状态，并且在可以使用任何查询之前，您需要重置池
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);

    MatrixState3D::pushMatrix();//总变换
...
void VulkanDemoApp::drawOcclusionObj() {
    ...
    for (int i = 0; i < CR - 1; i++) {
        ...
        objObjectZDT->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1705_muwen.bntex")]));
        MatrixState3D::popMatrix();
    }
...
void VulkanDemoApp::drawForQuery() {
    int indexForQuery = 0;
    for (int x = 0; x < CR; x++) {//X 方向遍历
        for (int y = 0; y < CR; y++) {//Y 方向遍历
            for (int z = 0; z < CR; z++) {//Z 方向遍历
                vkCmdBeginQuery(cmdBuffer, queryPool, indexForQuery, VK_QUERY_CONTROL_PRECISE_BIT);//开启遮挡查询
                MatrixState3D::pushMatrix();
                ...
                MatrixState3D::popMatrix();
                vkCmdEndQuery(cmdBuffer, queryPool, indexForQuery);//关闭遮挡查询
                indexForQuery++;//查询索引自增
            }
        }
    }
}
void VulkanDemoApp::drawForPresent() {
    ...
    for (int x = 0; x < CR; x++) {//X 方向遍历
        for (int y = 0; y < CR; y++) {//Y 方向遍历
            for (int z = 0; z < CR; z++) {//Z 方向遍历
                ...
                {
                    MatrixState3D::pushMatrix();
                    MatrixState3D::translate(x * span + xStart, y * span + yStart, z * span + zStart);
                    objObjectQT->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                          &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1705_qt.bntex")]));
                    MatrixState3D::popMatrix();
                }
...
void VulkanDemoApp::getQueryResult() {
    //将查询结果写入数组版本
    VkResult result = vkGetQueryPoolResults(
            ...
            VK_QUERY_RESULT_PARTIAL_BIT);//查询操作特定作业标志位
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsFQ = new ShaderQueueSuit_ForQuery(&device, renderPass, memoryProperties);
}
