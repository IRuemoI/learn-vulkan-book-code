...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 9.6", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    LandData *ld = FileUtil::loadHdtData("../textures/0906_default.bnhdt");
    landForDraw = new TexDrawableObject(ld->vData, ld->vCount * 8 * 4, ld->vCount, device, memoryProperties);
}
...
void VulkanDemoApp::flushTexToDesSet() const {//将纹理等数据与描述集关联的方法
    for (int i = 0; i < TextureManager::texNames.size() / 6; i++) {
        ...
        vkUpdateDescriptorSets(device, 7, sqsCT->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0906_default_c.bntex")]));
