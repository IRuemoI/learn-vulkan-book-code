...
#include <assert.h>
void ShaderQueueSuit_ForCalAoFactor::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_ForCalAoFactor::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_ForCalAoFactor::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_ForCalAoFactor::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_ForCalAoFactor::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_ForCalAoFactor::createShaderModule(VkDevice &device) {
    SpvData spvVertData = FileUtil::loadSPV("./shaders/calAoFactor.vert.spv");
    SpvData spvFragData = FileUtil::loadSPV("./shaders/calAoFactor.frag.spv");
...
void ShaderQueueSuit_ForCalAoFactor::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_ForCalAoFactor::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_ForCalAoFactor::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_ForCalAoFactor::ShaderQueueSuit_ForCalAoFactor(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
