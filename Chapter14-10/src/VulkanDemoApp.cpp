...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initVulkan() {
    ...
    create_vulkan_aoFactorBuffer();
    create_render_pass_screen();
    create_render_pass_self();
    create_render_pass_aoFactor();
...
void VulkanDemoApp::cleanup() {
    ...
    destroy_render_pass_aoFactor();
    destroyVulkanDepthBuffer();
    destroy_vulkan_aoFactorBuffer();
...
void VulkanDemoApp::create_render_pass_aoFactor() {
    ...
    rp_info.dependencyCount = 0;
    rp_info.pDependencies = nullptr;
    VkResult result = vkCreateRenderPass(device, &rp_info, nullptr, &renderPass_aoFactor);
    assert(result == VK_SUCCESS);
    ...
    rp_begin_aoFactor.pNext = nullptr;
...
void VulkanDemoApp::destroy_render_pass_aoFactor() {
    vkDestroyRenderPass(device, renderPass_aoFactor, nullptr);
}
...
void VulkanDemoApp::create_frame_buffer_self() {
    ...
    fb_info_ao.layers = 1;
    result = vkCreateFramebuffer(device, &fb_info_ao, nullptr, &aoFactorTexFramebuffer);
    assert(result == VK_SUCCESS);
}
void VulkanDemoApp::destroyFrameBuffer() {
    ...
    vkDestroyFramebuffer(device, aoFactorTexFramebuffer, nullptr);
    printf("销毁帧缓冲成功！\n");
}
...
void VulkanDemoApp::initMatrixAndLight() {
    ...
    MatrixState2D::setProjectOrtho(-ratio, ratio, -1, 1, 1.5f, 10000);
...
void VulkanDemoApp::flushUniformBufferForToScreen() {
    float fragmentUniformData[69] = {CameraUtil::camera9Para[0], CameraUtil::camera9Para[1], CameraUtil::camera9Para[2], 1.0};
...
void VulkanDemoApp::flushUniformBufferForCalAoFactor() {
    float fragmentUniformData[20] = {CameraUtil::camera9Para[0], CameraUtil::camera9Para[1], CameraUtil::camera9Para[2], 1.0};
    memcpy(&fragmentUniformData[4], MatrixState3D::getZHMatrix(), sizeof(float) * 16);
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsFCAF->memUniformBuf, 0, sqsFCAF->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsFCAF->bufferByteCount);
    vkUnmapMemory(device, sqsFCAF->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSetForCalAoFactor() {
    ...
    vkUpdateDescriptorSets(device, 3, sqsFCAF->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex() {
    ...
    skyboxObjForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("dx1.bntex"))]));
...
void VulkanDemoApp::drawForCalAoFactor() {
    rp_begin_aoFactor.framebuffer = aoFactorTexFramebuffer;
    vkResetCommandBuffer(cmdBuffer, 0);
    VkResult result = vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);
    flushTexToDesSetForCalAoFactor();
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin_aoFactor, VK_SUBPASS_CONTENTS_INLINE);
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsFCAF = new ShaderQueueSuit_ForCalAoFactor(&device, renderPass_aoFactor, memoryProperties);
}
...
void VulkanDemoApp::create_vulkan_aoFactorBuffer() {
    VkFormatProperties aoFactorFormatProps;
    VkImageCreateInfo image_info = {};
    vkGetPhysicalDeviceFormatProperties(gpus[USED_GPU_INDEX], VK_FORMAT_R32G32B32A32_SFLOAT, &aoFactorFormatProps);
    ...
    image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    image_info.flags = 0;
    VkResult result = vkCreateImage(device, &image_info, nullptr, &(aoFactorImage));
    assert(result == VK_SUCCESS);
    VkMemoryAllocateInfo mem_alloc = {};
    mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    mem_alloc.pNext = nullptr;
    ...
    mem_alloc.memoryTypeIndex = 0;
    VkMemoryRequirements mem_reqs;
    vkGetImageMemoryRequirements(device, aoFactorImage, &mem_reqs);
    mem_alloc.allocationSize = mem_reqs.size;
    VkFlags requirements_mask = 0;
    bool flag = memoryTypeFromProperties(memoryProperties, mem_reqs.memoryTypeBits, requirements_mask, &mem_alloc.memoryTypeIndex);
    assert(flag);
    printf("确定内存类型成功 类型索引为%d\n", mem_alloc.memoryTypeIndex);
    result = vkAllocateMemory(device, &mem_alloc, nullptr, &mem_aoFactor);
    assert(result == VK_SUCCESS);
    result = vkBindImageMemory(device, aoFactorImage, mem_aoFactor, 0);
    assert(result == VK_SUCCESS);
    VkImageViewCreateInfo view_info = {};
    view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    view_info.pNext = nullptr;
    ...
    result = vkCreateImageView(device, &view_info, nullptr, &aoFactorImageView);
    assert(result == VK_SUCCESS);
}
void VulkanDemoApp::destroy_vulkan_aoFactorBuffer() {
    vkDestroyImageView(device, aoFactorImageView, nullptr);
    vkDestroyImage(device, aoFactorImage, nullptr);
    vkFreeMemory(device, mem_aoFactor, nullptr);
}