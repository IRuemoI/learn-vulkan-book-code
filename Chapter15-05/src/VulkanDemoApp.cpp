...
float VulkanDemoApp::maxAngle = 30.0f;
float VulkanDemoApp::objScale = 0.0241f;
float VulkanDemoApp::objBeiZi = 0.02f;
float VulkanDemoApp::tx = 0;
float VulkanDemoApp::ty = 22;
float VulkanDemoApp::tz = 40;
float VulkanDemoApp::blurWidth = 60.0f;
float VulkanDemoApp::blurPosition = 30.0f;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1505_";
    floor = LoadUtil::loadFromFile(mp + (string) ("floor.obj"), device, memoryProperties);
    roof = LoadUtil::loadFromFile(mp + (string) ("roof.obj"), device, memoryProperties);
    wall = LoadUtil::loadFromFile(mp + (string) ("wall.obj"), device, memoryProperties);
    drawForWindow = LoadUtil::loadFromFile(mp + (string) ("window.obj"), device, memoryProperties);
    door = LoadUtil::loadFromFile(mp + (string) ("door.obj"), device, memoryProperties);
    doorframe = LoadUtil::loadFromFile(mp + (string) ("doorframe.obj"), device, memoryProperties);
    pillarY = LoadUtil::loadFromFile(mp + (string) ("pillarY.obj"), device, memoryProperties);
    pillarW = LoadUtil::loadFromFile(mp + (string) ("pillarW.obj"), device, memoryProperties);
    platform = LoadUtil::loadFromFile(mp + (string) ("platform.obj"), device, memoryProperties);
    balckboard = LoadUtil::loadFromFile(mp + (string) ("balckboard.obj"), device, memoryProperties);
    flag = LoadUtil::loadFromFile(mp + (string) ("flag.obj"), device, memoryProperties);
    chair = LoadUtil::loadFromFile(mp + (string) ("chair.obj"), device, memoryProperties);
    whiteboard = LoadUtil::loadFromFile(mp + (string) ("whiteboard.obj"), device, memoryProperties);
    desk = LoadUtil::loadFromFile(mp + (string) ("desk.obj"), device, memoryProperties);
    glass = LoadUtil::loadFromFile(mp + (string) ("glass.obj"), device, memoryProperties);
    galsshandle = LoadUtil::loadFromFile(mp + (string) ("galsshandle.obj"), device, memoryProperties);
    glasscover = LoadUtil::loadFromFile(mp + (string) ("glasscover.obj"), device, memoryProperties);
    SkyCloudData::initData();
    sky = new DrawableObject(SkyCloudData::vdata, SkyCloudData::dataByteCount, SkyCloudData::vCount, device, memoryProperties);
}
void VulkanDemoApp::destroyDrawableObject() {
    delete wall;
    delete floor;
    delete roof;
    delete drawForWindow;
    delete door;
    delete doorframe;
    delete pillarY;
    delete glass;
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    ...
    result = vkMapMemory(device, sqsCTLB->memUniformBuf, 0, sqsCTLB->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTLB->bufferByteCount);
    vkUnmapMemory(device, sqsCTLB->memUniformBuf);
    float fragmentUniformData[1] =
            {
                    0.9f};
    result = vkMapMemory(device, sqsSky->memUniformBuf, 0, sqsSky->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsSky->bufferByteCount);
    vkUnmapMemory(device, sqsSky->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSetForToTex() {
    ...
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCTLB->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNamesForSky.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsSky->writes, 0, nullptr);
    }
}
void VulkanDemoApp::drawSceneToTex() {
    const char *tp = "../textures/1505_";
    rp_begin.framebuffer = selfTexFramebuffer;
    vkResetCommandBuffer(cmdBuffer, 0);
    VkResult result = vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);
    VulkanDemoApp::flushUniformBufferForToTex();
    VulkanDemoApp::flushTexToDesSetForToTex();
    ...
    glass->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("glass.bntex"))]));
    galsshandle->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                          &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("roof.bntex"))]));
    ...
    floor->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("floor.bntex"))]));
    roof->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                   &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("roof.bntex"))]));
    drawForWindow->drawSelf(cmdBuffer, sqsCTLB->pipelineLayout, sqsCTLB->pipeline,
                            &(sqsCTLB->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("window.bntex"))]));
    ...
    door->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                   &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("whiteboard.bntex"))]));
    ...
    flag->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                   &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("flag.bntex"))]));
    chair->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("chair.bntex"))]));
    whiteboard->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                         &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("blackboard.bntex"))]));
...
void VulkanDemoApp::drawSceneToScreen()//完成第二轮绘制的方法
{
    ...
    VulkanDemoApp::flushUniformBufferForToScreen();
    VulkanDemoApp::flushTexToDesSetForToScreen();
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-4.0f, 15, -66);
    MatrixState3D::scale(objBeiZi, objBeiZi, objBeiZi);
    ...
    drawForWindow->drawSelf(cmdBuffer, sqsD2D->pipelineLayout, sqsD2D->pipeline, &(sqsD2D->descSet[0]));//绘制窗户
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsCTLB = new ShaderQueueSuit_CommonTexLightBlend(&device, renderPass, memoryProperties);
    sqsD2D = new ShaderQueueSuit_TexBlur(&device, renderPass, memoryProperties);
    sqsSky = new ShaderQueueSuit_Sky(&device, renderPass, memoryProperties);
}
...
void VulkanDemoApp::destroy_vulkan_SelfColorBuffer() {
    ...
    vkFreeMemory(device, memColor, nullptr);
}
