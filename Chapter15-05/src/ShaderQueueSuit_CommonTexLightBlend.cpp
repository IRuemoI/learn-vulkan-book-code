...
#include <assert.h>
void ShaderQueueSuit_CommonTexLightBlend::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_CommonTexLightBlend::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_CommonTexLightBlend::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonTexLightBlend::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_CommonTexLightBlend::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_CommonTexLightBlend::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/commonTexLight.vert";
    const char *fs = "./shaders/commonTexLight.frag";
...
void ShaderQueueSuit_CommonTexLightBlend::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_CommonTexLightBlend::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_CommonTexLightBlend::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_CommonTexLightBlend::ShaderQueueSuit_CommonTexLightBlend(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
