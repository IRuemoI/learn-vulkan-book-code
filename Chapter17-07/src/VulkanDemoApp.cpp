...
float VulkanDemoApp::qsj1 = 0;
float VulkanDemoApp::qsj2 = 45;
float VulkanDemoApp::qsj3 = 90;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texForDraw = new TexDrawableObject(RectData::vData, RectData::dataByteCount, RectData::vCount, device, memoryProperties);
    VertData::genVertData();
    waterForDraw = new TexLightObject(VertData::vdata, VertData::dataByteCount, VertData::vCount, VertData::indexData, VertData::indexDataByteCount, VertData::indexCount, device, memoryProperties);
    objPool = LoadUtil::loadFromFile("../models/1707_poolnew.obj", device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    ...
    LightManager::setLightPosition(-SKY_BOX_HALF_EDGE + 900, -SKY_BOX_HALF_EDGE + 900, -SKY_BOX_HALF_EDGE + 300);
...
void VulkanDemoApp::flushUniformBufferForDraw() {
    ...
    VkResult result = vkMapMemory(device, sqsSTL->memUniformBuf, 0, sqsSTL->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);

    memcpy(pData, vertexUniformData, sqsSTL->bufferByteCount);

    vkUnmapMemory(device, sqsSTL->memUniformBuf);


    result = vkMapMemory(device, sqsLoad->memUniformBuf, 0, sqsLoad->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsLoad->bufferByteCount);
    vkUnmapMemory(device, sqsLoad->memUniformBuf);
}
...
void VulkanDemoApp::flushUniformBufferForBD() {

    qsj1 = myMod(float(qsj1 + 0.09), 360);

    qsj2 = myMod(float(qsj2 + 0.18), 360);

    qsj3 = myMod(float(qsj3 + 0.04), 360);
    ...
    VkResult result = vkMapMemory(device, sqsBD->memUniformBuf, 0, sqsBD->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);

    memcpy(pData, vertexUniformData, sqsBD->bufferByteCount);

    vkUnmapMemory(device, sqsBD->memUniformBuf);
}


void VulkanDemoApp::flushTexToDesSetForDraw() {
    ...
    for (int i = 0; i < TextureManager::texNamesForCubeMap.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsSTL->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNamesForSkyBox.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsTex->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNamesForSkyBox.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsLoad->writes, 0, nullptr);
    }
}


void VulkanDemoApp::flushTexToDesSetForBD() {
    ...
    vkUpdateDescriptorSets(device, 3, sqsBD->writes, 0, nullptr);
}


void VulkanDemoApp::flushTexToDesSetForNormal() {

    sqsNormal->writes[0].dstSet = sqsNormal->descSet[0];
    sqsNormal->writes[0].pBufferInfo = &(waterForDraw->vertexDataBufferInfoCompute);
    vkUpdateDescriptorSets(device, 1, sqsNormal->writes, 0, nullptr);
}

void VulkanDemoApp::drawTask() {
    ...
    VulkanDemoApp::flushUniformBufferForDraw();

    VulkanDemoApp::flushTexToDesSetForDraw();
    ...
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline, &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_back.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 0, -SKY_BOX_HALF_EDGE);
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline,
                         &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_front.bntex")]));
    MatrixState3D::popMatrix();
    ...
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline,
                         &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_left.bntex")]));
    MatrixState3D::popMatrix();
    ...
    MatrixState3D::rotate(-90, 0, 1, 0);
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline,
                         &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_right.bntex")]));
    MatrixState3D::popMatrix();
    ...
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline,
                         &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_up.bntex")]));
    MatrixState3D::popMatrix();
    ...
    MatrixState3D::rotate(90, 1, 0, 0);
    texForDraw->drawSelf(cmdBuffer, sqsTex->pipelineLayout, sqsTex->pipeline,
                         &(sqsTex->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_skycubemap_down.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, -SKY_BOX_HALF_EDGE + 200, 0);
    waterForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline,
                           &(sqsSTL->descSet[TextureManager::getVkDescriptorSetIndexForCubemap("../textures/1707_cube.bntexcube")]));
    MatrixState3D::popMatrix();
    ...
    objPool->drawSelf(cmdBuffer, sqsLoad->pipelineLayout, sqsLoad->pipeline,
                      &(sqsLoad->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1707_poolnew.bntex")]));
    MatrixState3D::popMatrix();
    ...
    result = vkQueueSubmit(queueGraphics, 1, &submit_info[0], taskFinishFence);
...
void VulkanDemoApp::calTaskBD() {

    vkResetCommandBuffer(cmdBuffer, 0);

    VkResult result = vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);

    VulkanDemoApp::flushUniformBufferForBD();

    VulkanDemoApp::flushTexToDesSetForBD();
    ...
    result = vkQueueSubmit(queueGraphics, 1, &submit_info[1], taskFinishFence);
...
void VulkanDemoApp::calTaskNormal() {

    vkResetCommandBuffer(cmdBuffer, 0);

    VkResult result = vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);

    VulkanDemoApp::flushTexToDesSetForNormal();
...
void VulkanDemoApp::initPipeline() {
    sqsSTL = new ShaderQueueSuit_SingleTexLight(&device, renderPass, memoryProperties);
    sqsBD = new ShaderQueueSuit_ComputeBD(&device, memoryProperties);
    sqsNormal = new ShaderQueueSuit_ComputeNormal(&device, memoryProperties);
    sqsTex = new ShaderQueueSuit_CommonTex(&device, renderPass, memoryProperties);
    sqsLoad = new ShaderQueueSuit_CommonTexLoad(&device, renderPass, memoryProperties);
}
