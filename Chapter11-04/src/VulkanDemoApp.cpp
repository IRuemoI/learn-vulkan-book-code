...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 11.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    std::vector<float> tVectorA = LoadUtil::loadFromFileVertexAndTex("../models/1104_laoying01.obj");//加载关键帧1 模型的顶点坐标和纹理坐标
    std::vector<float> tVectorB = LoadUtil::loadFromFileVertex("../models/1104_laoying02.obj");      //加载关键帧2 模型的顶点坐标
    std::vector<float> tVectorC = LoadUtil::loadFromFileVertex("../models/1104_laoying03.obj");      //加载关键帧3 模型的顶点坐标
    ...
    LYobjDraw1 = new ObjObject(vdataIn, dataByteCount, vCount, device, memoryProperties);//创建绘制用物体
}
...
void VulkanDemoApp::drawObject() {
    ...
    LYobjDraw1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1104_laoying.bntex")]));
