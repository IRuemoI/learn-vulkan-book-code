...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.3V1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObjectPM = LoadUtil::loadFromFile("../models/1403_pm.obj", device, memoryProperties);
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    float vertexUniformData[6] = {LightManager::lx, LightManager::ly, LightManager::lz, 1.0, 1.5, 1000};
    uint8_t *pData;
...
void VulkanDemoApp::drawSceneToTex() {
    ...
    float ratio = (float) screenWidth / (float) screenHeight;
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.5f, 1000);
    objObjectPM->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
    ...
    treeForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87, 0, 50);
    carForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    sqsCTL = new ShaderQueueSuit_Shadow(&device, renderPass, memoryProperties);
    sqsD2D = new ShaderQueueSuit_CommonTex(&device, renderPass, memoryProperties);
}
