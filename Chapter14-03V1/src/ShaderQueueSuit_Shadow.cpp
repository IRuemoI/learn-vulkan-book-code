...
#include <assert.h>
#include <string>
void ShaderQueueSuit_Shadow::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_Shadow::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_Shadow::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_Shadow::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_Shadow::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_Shadow::createShaderModule(VkDevice &device) {
    std::string vertStr = FileUtil::loadAssetStr("shaders/shadow.vert");
    std::string fragStr = FileUtil::loadAssetStr("shaders/shadow.frag");
...
void ShaderQueueSuit_Shadow::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_Shadow::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_Shadow::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_Shadow::ShaderQueueSuit_Shadow(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
