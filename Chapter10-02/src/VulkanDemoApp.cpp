...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 10.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initVulkan() {
    ...
    create_vulkan_DepthStencilBuffer();// 创建深度缓冲
...
void VulkanDemoApp::cleanup() {
    ...
    destroy_vulkan_DepthStencilBuffer();// 销毁深度缓冲相关
...
// 创建深度缓冲相关
void VulkanDemoApp::create_vulkan_DepthStencilBuffer() {
    depthStencilFormat = VK_FORMAT_D24_UNORM_S8_UINT;
    VkImageCreateInfo image_info = {};
    vkGetPhysicalDeviceFormatProperties(gpus[USED_GPU_INDEX], depthStencilFormat, &depthStencilFormatProps);
    ...
    view_info.flags = 0;
    VkResult result = vkCreateImage(device, &image_info, nullptr, &depthStencilImage);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqs;
    vkGetImageMemoryRequirements(device, depthStencilImage, &mem_reqs);
    ...
    result = vkAllocateMemory(device, &mem_alloc, nullptr, &memDepthStencil);
    assert(result == VK_SUCCESS);
    result = vkBindImageMemory(device, depthStencilImage, memDepthStencil, 0);
    assert(result == VK_SUCCESS);
    view_info.image = depthStencilImage;
    result = vkCreateImageView(device, &view_info, nullptr, &depthStencilImageView);
    assert(result == VK_SUCCESS);
}

void VulkanDemoApp::destroy_vulkan_DepthStencilBuffer() const {
    vkDestroyImageView(device, depthStencilImageView, nullptr);
    vkDestroyImage(device, depthStencilImage, nullptr);
    vkFreeMemory(device, memDepthStencil, nullptr);
    printf("销毁深度缓冲相关成功!\n");
}
...
void VulkanDemoApp::createFrameBuffer() {
    ...
    attachments[1] = depthStencilImageView;// 给定深度图像视图
...
void VulkanDemoApp::createDrawableObject() {
    ...
    planeForDraw = LoadUtil::loadFromFile("../models/0909_plane.obj", device, memoryProperties);
    ballForDraw = LoadUtil::loadFromFile("../models/0909_ball.obj", device, memoryProperties);
}
// 销毁绘制用物体
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    uint8_t *pDataN;
    result = vkMapMemory(device, sqsND->memUniformBuf, 0, sqsND->bufferByteCount, 0, (void **) &pDataN);
    assert(result == VK_SUCCESS);
    memcpy(pDataN, vertexUniformData, sqsND->bufferByteCount);
    vkUnmapMemory(device, sqsND->memUniformBuf);

    uint8_t *pDataM;
    result = vkMapMemory(device, sqsM->memUniformBuf, 0, sqsM->bufferByteCount, 0, (void **) &pDataM);
    assert(result == VK_SUCCESS);
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsND->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
...
void VulkanDemoApp::drawObject() {
    ...
    planeForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,//绘制木地板
                           &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_mdb.bntex")]), false);
    MatrixState3D::popMatrix();                                             //恢复现场
    MatrixState3D::pushMatrix();                                            //保护现场
    MatrixState3D::scale(1, -1, 1);                                         //执行缩放变换
    ...
    ballForDraw->drawSelf(cmdBuffer, sqsND->pipelineLayout, sqsND->pipeline,//绘制镜像球
                          &(sqsND->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_ball.bntex")]), true);
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(0.7, 1.0, 0.5);
    planeForDraw->drawSelf(cmdBuffer, sqsM->pipelineLayout, sqsM->pipeline,
                           &(sqsM->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_mdbtm.bntex")]), false);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, bfc->currentY + 0.4, 0);
    ballForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                          &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_ball.bntex")]), false);
    MatrixState3D::popMatrix();//恢复现场
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsND = new ShaderQueueSuit_NoDepth(&device, renderPass, memoryProperties);
