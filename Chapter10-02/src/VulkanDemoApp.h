...
public:
    ...
    VkFormat depthStencilFormat;// 深度图像格式
    VkFormatProperties depthStencilFormatProps;// 物理设备支持的深度格式属性
    VkImage depthStencilImage;// 深度缓冲图像
    VkPhysicalDeviceMemoryProperties memoryProperties;// 物理设备内存属性
    VkDeviceMemory memDepthStencil;// 深度缓冲图像对应的内存
    VkImageView depthStencilImageView;// 深度缓冲图像视图
    ...
    ShaderQueueSuit_NoDepth *sqsND;
    ShaderQueueSuit_Mirror *sqsM;
    BallForControl *bfc;
    ObjObject *ballForDraw;
    ...
    void create_vulkan_DepthStencilBuffer();// 创建深度缓冲相关
    ...
    void destroy_vulkan_DepthStencilBuffer() const;// 销毁深度缓冲相关
