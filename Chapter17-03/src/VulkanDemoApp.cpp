...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    planet1 = new Planet(1, false);
    planet2 = new Planet(2, false);
    planet3 = new Planet(3, false);
    planet4 = new Planet(4, true);
    planet5 = new Planet(5, true);
    planet6 = new Planet(6, true);
    planet1->initInstanceData();
    planet2->initInstanceData();
    planet3->initInstanceData();
    planet4->initInstanceData();
    planet5->initInstanceData();
    planet6->initInstanceData();
    planetVector.push_back(planet1);
    planetVector.push_back(planet2);
    planetVector.push_back(planet3);
    planetVector.push_back(planet4);
    planetVector.push_back(planet5);
    planetVector.push_back(planet6);
}
...
void VulkanDemoApp::flushTexToDesSet() {
    ...
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsQiu->writes, 0, nullptr);
    }
}
void VulkanDemoApp::drawObject() {
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1703_qiu.bntex")]));
    MatrixState3D::popMatrix();

    PlanetConstant::perAngle += 5.0f;
    for (auto planet: planetVector) {
        planet->update();
            ...
            switch (planet->id) {
                case 1:
                    objQiu->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline,
                                     &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1703_stone.bntex")]));
                    break;
                case 2:
                    objQiu->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline,
                                     &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1703_stone2.bntex")]));
                    break;
                case 3:
                    objQiu->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline,
                                     &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1703_stone3.bntex")]));
                    break;
                case 4:
                    objQiu->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline,
                                     &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1703_stone.bntex")]));
