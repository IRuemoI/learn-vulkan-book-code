...
float VulkanDemoApp::yAngle = 90;
int VulkanDemoApp::depthOffsetFlag = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.13", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    colorRectG = new DrawableObjectCommon(ColorRect::vdataG, ColorRect::dataByteCount, ColorRect::vCount, device, memoryProperties);
    colorRectR = new DrawableObjectCommon(ColorRect::vdataR, ColorRect::dataByteCount, ColorRect::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio * 225.0f, ratio * 225.0f, -225.0f, 225.0f, 900.0f, 10000.0f);
...
void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    ...
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData,
           sqsCL->bufferByteCount);             // 将最终矩阵数据拷贝进显存
...
void VulkanDemoApp::drawObject() {
    ...
    // 获取交换链中的当前帧索引
    VkResult result =
            vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAcquiredSemaphore, VK_NULL_HANDLE, &currentBuffer);
    if (result != VK_SUCCESS) {
    ...
    colorRectG->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制青色矩形
    ...
    switch (depthOffsetFlag) { //根据索引设置黄色矩形深度偏移参数
        case 0:
            break;
        case 1:
            vkCmdSetDepthBias(cmdBuffer, -1.0, -3.0, -2.0);
            break;//黄色矩形深度值减小
        case 2:
            vkCmdSetDepthBias(cmdBuffer, 1.0, 3.0, 2.0);
            break;
    ...
    colorRectR->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制黄色矩形
