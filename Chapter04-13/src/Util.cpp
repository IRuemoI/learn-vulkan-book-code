...
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
    ...
    if (left_mouse_down) {
        ...
        if (!isClick) {//若为滑动操作
            VulkanDemoApp::yAngle += VulkanDemoApp::cursorDeltaX * 180 / 200;//计算绕y轴转角
            VulkanDemoApp::xAngle += VulkanDemoApp::cursorDeltaY * 180 / 200;//计算绕x 轴转角
            preX = VulkanDemoApp::currentCursorX;//更新触控点x坐标
            preY = VulkanDemoApp::currentCursorY;//更新触控点y坐标
        }
    }
}
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (isClick) {//若为点击操作
            VulkanDemoApp::depthOffsetFlag = ++VulkanDemoApp::depthOffsetFlag % 3;//切换depthOffsetFlag 的值
            printf("正在切换深度偏移的值\n");
