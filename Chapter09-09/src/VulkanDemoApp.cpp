...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 9.9", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::drawObject() {
    ...
    planeForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                           &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_mdb.bntex")]), false);//绘制地板(反射面)
    MatrixState3D::popMatrix();                                                                                             //恢复现场
    MatrixState3D::pushMatrix();                                                                                            //保护现场
    MatrixState3D::scale(1, -1, 1);                                                                                         //通过缩放(Y 轴置反)实现镜像
    MatrixState3D::translate(0, bfc->currentY + 0.4, 0);                                                                    //执行平移
    ballForDraw->drawSelf(cmdBuffer, sqsND->pipelineLayout, sqsND->pipeline,
                          &(sqsND->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_ball.bntex")]), true);//绘制镜像体
    MatrixState3D::popMatrix();                                                                                            //恢复现场
    MatrixState3D::pushMatrix();                                                                                           //保护现场
    MatrixState3D::translate(0, bfc->currentY + 0.4, 0);                                                                   //执行平移
    ballForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                          &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_ball.bntex")]), false);//绘制物体本身
    MatrixState3D::popMatrix();                                                                                             //恢复现场
