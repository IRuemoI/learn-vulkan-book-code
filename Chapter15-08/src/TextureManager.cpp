...
const char *tp = "../textures/1508_";
std::vector<std::string> TextureManager::texNames = {tp + (string) ("Head_256x256x256.bn3dtex"), tp + (string) ("Head_256x256x256.bntex")};
std::vector<std::string> TextureManager::texNames2D = {tp + (string) ("Head_256x256x256.bntex")};
std::vector<std::string> TextureManager::texNames3D = {tp + (string) ("Head_256x256x256.bn3dtex")};
...
void TextureManager::init_SPEC_3D_Textures(std::string texName, VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics, VkFormat format, ThreeDTexDataObject *ctdo) {
...
void TextureManager::initTextures(VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics) {
    ...
    for (int i = 0; i < texNames.size(); i++) {
        ...
        if (sa[1].compare("bntex") == 0) {
            ...
            init_SPEC_3D_Textures(texName, device, gpu, memoryProperties, cmdBuffer, queueGraphics, VK_FORMAT_R16_UNORM, ctdo);
        }
    }
}
