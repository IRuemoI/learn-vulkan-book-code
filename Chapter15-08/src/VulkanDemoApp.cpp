...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.8", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::create_vulkan_SelfColorBufferSpec(int index) {
...
void VulkanDemoApp::createRenderPass() {
    ...
    int i = 0;
    printf("[%d]支持的hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh格式为%d\n", i, formats[i]);
...
void VulkanDemoApp::createDrawableObject() {
    LFTData::getVData();
    lftObject = new CommonObject(LFTData::vdata, LFTData::dataByteCount, LFTData::vCount, device, memoryProperties);
...
void VulkanDemoApp::initMatrix() {
    ...
    float scaleView = 0.6f;
    MatrixState3D::setProjectFrustum(-ratio * scaleView, ratio * scaleView, -1 * scaleView, 1 * scaleView, 3.0f, 1000);
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    ...
    vkMapMemory(device, sqsCLSSZ->memUniformBuf, 0, sqsCLSSZ->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCLSSZ->bufferByteCount);
    vkUnmapMemory(device, sqsCL->memUniformBuf);
    vkUnmapMemory(device, sqsCLSSZ->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSetForToTex() {
    sqsCL->writes[0].dstSet = sqsCL->descSet[0];
    vkUpdateDescriptorSets(device, 1, sqsCL->writes, 0, nullptr);
    sqsCLSSZ->writes[0].dstSet = sqsCLSSZ->descSet[0];
    vkUpdateDescriptorSets(device, 1, sqsCLSSZ->writes, 0, nullptr);
}
...
void VulkanDemoApp::drawSceneToTexSpec(VkFramebuffer framebuffer, VkPipelineLayout &pipelineLayout, VkPipeline &pipeline, VkDescriptorSet *desSetPointer) {
    ...
    lftObject->drawSelf(cmdBuffer, pipelineLayout, pipeline, desSetPointer);
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsCLSSZ = new ShaderQueueSuit_SSZ(&device, renderPass, memoryProperties);
