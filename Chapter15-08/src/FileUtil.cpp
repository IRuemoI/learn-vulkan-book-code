...
ThreeDTexDataObject *FileUtil::load3DTexData(string filename) {
    unsigned char *buf = new unsigned char[4];
    char c_file[100];
    strcpy(c_file, filename.c_str());
    FILE *fpPhoto;
    ...
    unsigned char *data = new unsigned char[width * height * depth * 2];
    fread(data, width * height * depth * 2, 1, fpPhoto);
