...
CommonObject *LoadUtil::loadFromFile(const string &vname, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    CommonObject *lo;
    vector<float> alv;
    ...
    lo = new CommonObject(vdataIn, dataByteCount, vCount, device, memoryProperties);
    return lo;
}
