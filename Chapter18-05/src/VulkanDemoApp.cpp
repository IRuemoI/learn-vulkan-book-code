...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 18.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::flushTexToDesSet() const {//将纹理等数据与描述集关联的方法
    ...
    for (int i = 0; i < TextureManager::texNamesPair.size() / 2; i++) {                                             //遍历所有地球纹理组
        ...
        sqsDTL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesPair[i * 2 + 0]]);//写入属性1对应的纹理图像信息（白天）
        sqsDTL->writes[2].dstSet = sqsDTL->descSet[i];                                                              //更新描述集对应的写入属性2(纹理)
        sqsDTL->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesPair[i * 2 + 1]]);//写入属性2对应的纹理图像信息（黑夜）
        vkUpdateDescriptorSets(device, 3, sqsDTL->writes, 0, nullptr);                                              //更新描述集
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    ballForDraw->drawSelf(cmdBuffer, sqsDTL->pipelineLayout, sqsDTL->pipeline, &(sqsDTL->descSet[0]));                                                                                      //绘制地球
    MatrixState3D::popMatrix();                                                                                                                                                             //恢复现场
    MatrixState3D::translate(180, 0, 0);                                                                                                                                                    //沿 x 轴平移（地月距离）
    MatrixState3D::rotate(mAngle, 0, 1, 0);                                                                                                                                                 //绕 y 轴旋转（月球自转）
    ballForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline, &(sqsSTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/0606_moon.bntex")]));//绘制月球
    MatrixState3D::popMatrix();                                                                                                                                                             //恢复现场
    MatrixState3D::pushMatrix();                                                                                                                                                            //保护现场
