...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 22, 0, 0, 0, 0, 1, 0);              // 初始化摄像机
...
void VulkanDemoApp::drawObject() {
    ...
    for (int i = 0; i < 3; i++) {                                             //循环列
        for (int j = 0; j < 3; j++) {                                         //循环行
            ...
            //绘制物体（正方形）
            texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0605_mipmap.bntex")]), currLodLevel);//传入纹理采样细节级别
