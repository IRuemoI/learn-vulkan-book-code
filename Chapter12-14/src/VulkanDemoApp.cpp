...
std::mutex VulkanDemoApp::myLock;


bool VulkanDemoApp::isParticleThreadShouldClose = false;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 12.14", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::mainLoop() {
    ...
    //开辟一个线程 负责不断更新粒子系统
    particleThread = std::thread(&VulkanDemoApp::updateParticleSystem, this);
    ...
    isParticleThreadShouldClose = true;
...
void VulkanDemoApp::createDrawableObject() {
    LYobjB = LoadUtil::loadFromFileVertexAndTex("../models/1214_brazier.obj", device, memoryProperties);
    FlatData::genVertexData();
    LYobjDiban = new ObjObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryProperties);
    //循环创建粒子系统数据和粒子系统绘制类
    for (int i = 0; i < 4; i++) {
        particleVector.push_back(new ParticleSystem(COUNT[i], ParticlePosition[i][0], ParticlePosition[i][1], i));
        particleVector[i]->genVertexData();
        LYobjParticle[i] = new ParticleForDraw(particleVector[i]->vdata, particleVector[i]->dataByteCount, particleVector[i]->vCount, device, memoryProperties);
    }
}
...
void VulkanDemoApp::initMatrix() {
    MatrixState3D::setInitStack();
    float ratio = (float) screenWidth / (float) screenHeight;
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.5f, 1000);
}

void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    CameraUtil::calCamera(0, 0);
    float uniformData[] = {5.0f};
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsParticle->memUniformBuf, 0, sqsParticle->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticle->bufferByteCount);
    vkUnmapMemory(device, sqsParticle->memUniformBuf);

    result = vkMapMemory(device, sqsParticleTwo->memUniformBuf, 0, sqsParticleTwo->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleTwo->bufferByteCount);
    vkUnmapMemory(device, sqsParticleTwo->memUniformBuf);

    result = vkMapMemory(device, sqsParticleThree->memUniformBuf, 0, sqsParticleThree->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleThree->bufferByteCount);
    vkUnmapMemory(device, sqsParticleThree->memUniformBuf);

    result = vkMapMemory(device, sqsParticleFour->memUniformBuf, 0, sqsParticleFour->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleFour->bufferByteCount);
    vkUnmapMemory(device, sqsParticleFour->memUniformBuf);
}

void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCT->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsParticle->writes, 0, nullptr);
        ...
        vkUpdateDescriptorSets(device, 2, sqsParticleTwo->writes, 0, nullptr);
        ...
        vkUpdateDescriptorSets(device, 2, sqsParticleThree->writes, 0, nullptr);
        ...
        vkUpdateDescriptorSets(device, 2, sqsParticleFour->writes, 0, nullptr);
    }
}

void VulkanDemoApp::updateParticleSystem() {
    while (!isParticleThreadShouldClose) {
        //更新每一个粒子系统
...
void VulkanDemoApp::drawObject() {
    ...
    LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1214_wall.bntex")]));
    MatrixState3D::popMatrix();
    //绘制架子
    const float penDistance = 7.0f;
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(penDistance, 0, penDistance);
    LYobjB->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1214_brazier.bntex")]));
    MatrixState3D::popMatrix();
    ...
    for (auto oneP: particleVector) {
        ...
        switch (oneP->id) {
            case 0:
                LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticle->pipelineLayout, sqsParticle->pipeline, &(sqsParticle->descSet[TextureManager::getVkDescriptorSetIndex(("../textures/1214_fire.bntex"))]), oneP->id);
                break;
            case 1:
                LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleTwo->pipelineLayout, sqsParticleTwo->pipeline, &(sqsParticleTwo->descSet[TextureManager::getVkDescriptorSetIndex(("../textures/1214_fire.bntex"))]), oneP->id);
                break;
            case 2:
                LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleThree->pipelineLayout, sqsParticleThree->pipeline, &(sqsParticleThree->descSet[TextureManager::getVkDescriptorSetIndex(("../textures/1214_fire.bntex"))]), oneP->id);
                break;
            case 3:
                LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleFour->pipelineLayout, sqsParticleFour->pipeline, &(sqsParticleFour->descSet[TextureManager::getVkDescriptorSetIndex(("../textures/1214_fire.bntex"))]), oneP->id);
                break;
        }
        MatrixState3D::popMatrix();
    }
...
void VulkanDemoApp::initPipeline() {
    sqsCT = new ShaderQueueSuit_CommonTex(&device, renderPass, memoryProperties);
    sqsParticle = new ShaderQueueSuit_CommonTexParticle(&device, renderPass, memoryProperties);
    sqsParticleTwo = new ShaderQueueSuit_CommonTexParticleTwo(&device, renderPass, memoryProperties);
    sqsParticleThree = new ShaderQueueSuit_CommonTexParticleThree(&device, renderPass, memoryProperties);
    sqsParticleFour = new ShaderQueueSuit_CommonTexParticleFour(&device, renderPass, memoryProperties);
}
...
void VulkanDemoApp::calculateDistance() {
    sort(particleVector.begin(), particleVector.end(), compare);
}
bool VulkanDemoApp::compare(ParticleSystem *pa, ParticleSystem *another) {
