...
float *VulkanDemoApp::MProjCameraMatrix;
float VulkanDemoApp::isToneMapping = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1507_";
    objObjectDog = LoadUtil::loadFromFile(mp + (string) ("dog.obj"), device, memoryProperties);
    objObjectPM = LoadUtil::loadFromFile(mp + (string) ("pm.obj"), device, memoryProperties);
}
    ...
    float vertexUniformDataShadow[40] =
            {
                    ...
                    LightManager::disX - LightManager::lx,
                    LightManager::disY - LightManager::ly,
                    LightManager::disZ - LightManager::lz,
                    1.0,
                    MProjCameraMatrix[0],
                    MProjCameraMatrix[1],
                    MProjCameraMatrix[2],
...
void VulkanDemoApp::drawObject() {
    ...
    objObjectPM->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1507_floor.bntex")]), 0);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(4.0f, 4.0f, 4.0f);
    objObjectDog->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                           &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1507_dog.bntex")]), 1);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(4.0f, 4.0f, 4.0f);
    objObjectDog->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                           &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1507_dog.bntex")]), 0);
    MatrixState3D::popMatrix();
