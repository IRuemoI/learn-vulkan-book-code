...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.15", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    CubeData::genCubeData();                                                                                                         // 生成待绘制对象的顶点数据和颜色数据
    objForDraw = new DrawableObjectCommonLight(CubeData::vdata, CubeData::dataByteCount, CubeData::vCount, device, memoryProperties);// 生成待绘制对象
}
...
void VulkanDemoApp::initMatrix() {
    MatrixState3D::setCamera(-16, 8, 45, 0, 0, 0, 0, 1.0, 0.0);                   // 初始化摄像机
    MatrixState3D::setInitStack();                                                // 初始化基本变换矩阵
    float ratio = (float) screenWidth / (float) screenHeight;                     // 求屏幕长宽比
    MatrixState3D::setProjectFrustum(-ratio * 0.8f, ratio * 1.2f, -1, 1, 20, 100);// 设置投影参数
}
...
void VulkanDemoApp::drawObject() {
    ...
    MatrixState3D::pushMatrix();                                                                  //保护现场
    objForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制第一个立方体
    MatrixState3D::popMatrix();                                                                   //恢复现场
    MatrixState3D::pushMatrix();                                                                  //保护现场
    MatrixState3D::translate(3.5f, 0, 0);                                                         //沿x方向平移3.5
    objForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制变换后的立方体
    MatrixState3D::popMatrix();                                                                   //恢复现场
    MatrixState3D::popMatrix();                                                                   //TODO:这里为什么要调用两次？
