...
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
    ...
    if (left_mouse_down) {
        ...
        printf("拖动鼠标的变化量:[X:%f,Y:%f]\n", VulkanDemoApp::cursorDeltaX, VulkanDemoApp::cursorDeltaY);
        if (abs((int) VulkanDemoApp::cursorDeltaX) > 3 || abs((int) VulkanDemoApp::cursorDeltaY) > 3) {
            isClick = false;
        }
    }
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (isClick) {
            VulkanDemoApp::topologyWay = (++VulkanDemoApp::topologyWay % ShaderQueueSuit_Common::topologyCount);
        }
        isClick = true;
