...
void ShaderQueueSuit_Common::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
    ...
    for (int i = 0; i < topologyCount; ++i) {//遍历数组中每个元素进行初始化
        ...
        switch (i) {//数组中的每个元素设置不同的图元组装方式
            case 0:
                ia[i].topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
                break;//点列表方式
            case 1:
                ia[i].topology = VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
                break;//线段列表方式
            case 2:
                ia[i].topology = VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
                break;//折线方式
        }
    }
    ...
    for (int i = 0; i < topologyCount; i++) {//为每种图元组装方式创建管线
        ...
        result = vkCreateGraphicsPipelines//创建管线
                (device, pipelineCache, 1, &pipelineInfo, nullptr, &pipelines[i]);
