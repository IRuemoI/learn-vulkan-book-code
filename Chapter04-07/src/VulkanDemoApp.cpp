...
int VulkanDemoApp::topologyWay = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createVulkanDevices() {
    ...
    vkGetPhysicalDeviceFeatures(gpus[USED_GPU_INDEX], &pdf);//获取指定设备支持的特性
    //判断是否支持wideLines特性
    if (pdf.wideLines == VK_TRUE) {
        printf("支持wideLines特性！\n");//打印调试信息
    } else {
        printf("不支持wideLines特性！\n");
    }
    ...
    deviceInfo.ppEnabledLayerNames = nullptr;                                             //启动的Layer名称列表
...
void VulkanDemoApp::createDrawableObject() {
    ObjectData::genVertexData();                                                                                                         // 待绘制对象的顶点数据和颜色数据
    objectForDraw = new DrawableObjectCommon(ObjectData::vdata, ObjectData::dataByteCount, ObjectData::vCount, device, memoryProperties);// 创建待绘制对象
}
...
void VulkanDemoApp::drawObject() {
    ...
    objectForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipelines[topologyWay], &(sqsCL->descSet[0]));// 绘制目标对象
