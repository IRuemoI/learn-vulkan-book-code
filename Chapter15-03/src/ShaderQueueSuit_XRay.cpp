...
void ShaderQueueSuit_XRay::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    bufferByteCount = sizeof(float) * 7;
...
void ShaderQueueSuit_XRay::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_XRay::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_XRay::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_XRay::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_XRay::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/XRay.vert";
    const char *fs = "./shaders/XRay.frag";
...
void ShaderQueueSuit_XRay::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_XRay::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_XRay::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_XRay::ShaderQueueSuit_XRay(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
