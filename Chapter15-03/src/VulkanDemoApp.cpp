...
float VulkanDemoApp::peopleScale = 8.0f;
float VulkanDemoApp::qiangDis = 85;
float VulkanDemoApp::cx = 0;
float VulkanDemoApp::cy = 20;
float VulkanDemoApp::cz = 35;
float VulkanDemoApp::cxPrevious = 0;
float VulkanDemoApp::cyPrevious = 0;
float VulkanDemoApp::czPrevious = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::create_frame_buffer_screen() {
    ...
    VkResult result = vkCreateFramebuffer(device, &fb_info, nullptr, &selfTexFramebufferPeople);
    assert(result == VK_SUCCESS);
}
void VulkanDemoApp::create_frame_buffer_self() {
    ...
    VkResult result = vkCreateFramebuffer(device, &Selffb_info, nullptr, &selfTexFramebuffer);
    assert(result == VK_SUCCESS);
}
...
void VulkanDemoApp::createDrawableObject() {
    ...
    people = LoadUtil::loadFromFile(mp + (string) ("people.obj"), device, memoryProperties);
    ...
    d2dA = new TexDrawableObject(vdataIn, 30 * 4, 6, device, memoryProperties);
}
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsCTLD->memUniformBuf, 0, sqsCTLD->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTLD->bufferByteCount);
    vkUnmapMemory(device, sqsCTLD->memUniformBuf);
    result = vkMapMemory(device, sqsCTL->memUniformBuf, 0, sqsCTL->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTL->bufferByteCount);
    vkUnmapMemory(device, sqsCTL->memUniformBuf);
    float vertexXRayUniformData[7] =
            {
                    CameraUtil::camera9Para[0], CameraUtil::camera9Para[1], CameraUtil::camera9Para[2], 1.0,
                    (float) screenWidth, (float) screenHeight, 0.9f};
    result = vkMapMemory(device, sqsXRay->memUniformBuf, 0, sqsXRay->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexXRayUniformData, sqsXRay->bufferByteCount);
    vkUnmapMemory(device, sqsXRay->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSetForToTex() {
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCTLD->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size(); i++) {
    ...
    vkUpdateDescriptorSets(device, 4, sqsXRay->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex()//用于完成第一轮绘制的方法
{
    const char *tp = "../textures/1503_";
    ...
    wall->drawSelf(cmdBuffer, sqsCTLD->pipelineLayout, sqsCTLD->pipeline,//绘制墙壁
                   &(sqsCTLD->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) ("brick.bntex"))]));
    ...
    MatrixState3D::translate(qiangDis, 80, 0);
    ...
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 80, qiangDis);
...
void VulkanDemoApp::drawPeopleToTex()//用于完成第二轮绘制的方法
{
    const char *tp = "../textures/1503_";
...
void VulkanDemoApp::drawSceneToScreen()//用于完成第三轮及第四轮绘制的方法
{
...
void VulkanDemoApp::initPipeline() {
    sqsCTLD = new ShaderQueueSuit_CommonTexLight_Depth(&device, renderPassSelf, memoryProperties);
    sqsCTL = new ShaderQueueSuit_CommonTexLight(&device, renderPassScreen, memoryProperties);
    sqsXRay = new ShaderQueueSuit_XRay(&device, renderPassScreen, memoryProperties);
    sqsD2D = new ShaderQueueSuit_TexRect(&device, renderPassScreen, memoryProperties);
}
