...
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
    ...
    if (left_mouse_down) {
        ...
        if (isClick) {
            CameraUtil::calCamera(-VulkanDemoApp::cursorDeltaY * 180 / 1000.0, -VulkanDemoApp::cursorDeltaX * 180 / 1000.0);
        }
    }
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        isClick = true;
        VulkanDemoApp::ifDiscard = !VulkanDemoApp::ifDiscard;//更新是否丢弃片元标志
        printf("鼠标松开\n");
    }
}
