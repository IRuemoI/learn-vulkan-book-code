...
bool VulkanDemoApp::ifDiscard = true;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 10.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::destroyVulkanDepthBuffer() const {
...
void VulkanDemoApp::createDrawableObject() {
    chuang = LoadUtil::loadFromFile("../models/1003_chuang.obj", device, memoryProperties);
    pm = LoadUtil::loadFromFile("../models/1003_pm.obj", device, memoryProperties);
    zhuomian = LoadUtil::loadFromFile("../models/1003_zhuomian.obj", device, memoryProperties);
    zhuotui = LoadUtil::loadFromFile("../models/1003_zhuotui.obj", device, memoryProperties);
}
// 销毁绘制用物体
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    uint8_t *pDataD;
    result = vkMapMemory(device, sqsD->memUniformBuf, 0, sqsD->bufferByteCount, 0, (void **) &pDataD);
    assert(result == VK_SUCCESS);
    memcpy(pDataD, vertexUniformData, sqsD->bufferByteCount);
    vkUnmapMemory(device, sqsD->memUniformBuf);
}
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsD->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    chuang->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                     &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_chuang.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(0.8, 0.8, 0.8);
    pm->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                 &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_dm.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    ...
    zhuomian->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                       &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_zhuomian.bntex")]));
    zhuotui->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                      &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_zhuotui.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    ...
    if (ifDiscard) {
        pm->drawSelf(cmdBuffer, sqsD->pipelineLayout, sqsD->pipeline,
                     &(sqsD->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_sha.bntex")]));
    } else {
        pm->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                     &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1003_sha.bntex")]));
    }
    MatrixState3D::popMatrix();
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsD = new ShaderQueueSuit_Discard(&device, renderPass, memoryProperties);
}
