...
#include <assert.h>
void ShaderQueueSuit_Discard::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_Discard::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_Discard::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_Discard::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_Discard::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_Discard::createShaderModule(VkDevice &device) {
    const char *ss = "./shaders/shaderDiscard.vert";
    const char *vs = "./shaders/shaderDiscard.frag";
...
void ShaderQueueSuit_Discard::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_Discard::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_Discard::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_Discard::ShaderQueueSuit_Discard(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
