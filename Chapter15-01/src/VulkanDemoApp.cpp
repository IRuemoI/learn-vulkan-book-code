...
float VulkanDemoApp::SPAN = 0.5f;
float VulkanDemoApp::TIMESPAN = 0.75f;
float VulkanDemoApp::cz = 100;
float VulkanDemoApp::targetz = 0;
bool VulkanDemoApp::isTreeMoveThreadShouldClose = false;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::mainLoop() {
    ...
    treeMoveThread = std::thread(&VulkanDemoApp::updatePosition, this);
    treeMoveThread.detach();
    ...
    isTreeMoveThreadShouldClose = true;
...
void VulkanDemoApp::create_vulkan_SelfColorBuffer() {
    ...
    image_info.flags = 0;
    for (int i = 0; i < 5; i++) {
        VkResult result = vkCreateImage(device, &image_info, nullptr, &(colorImage[i]));
        assert(result == VK_SUCCESS);
        VkMemoryAllocateInfo mem_alloc = {};
        mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        mem_alloc.pNext = nullptr;
        ...
        mem_alloc.memoryTypeIndex = 0;
        VkMemoryRequirements mem_reqs;
        vkGetImageMemoryRequirements(device, colorImage[i], &mem_reqs);
        mem_alloc.allocationSize = mem_reqs.size;
        VkFlags requirements_mask = 0;
        bool flag = memoryTypeFromProperties(memoryProperties, mem_reqs.memoryTypeBits, requirements_mask, &mem_alloc.memoryTypeIndex);
        assert(flag);
        printf("确定内存类型成功 类型索引为%d", mem_alloc.memoryTypeIndex);
        result = vkAllocateMemory(device, &mem_alloc, nullptr, &(memColor[i]));
        assert(result == VK_SUCCESS);
        result = vkBindImageMemory(device, colorImage[i], memColor[i], 0);
        assert(result == VK_SUCCESS);
        VkImageViewCreateInfo view_info = {};
        view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        view_info.pNext = nullptr;
        ...
        result = vkCreateImageView(device, &view_info, nullptr, &(colorImageView[i]));
        assert(result == VK_SUCCESS);
    }
}
...
void VulkanDemoApp::createFrameBuffer() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        VkResult result = vkCreateFramebuffer(device, &fb_info, nullptr, &selfTexFramebuffer[i]);
        assert(result == VK_SUCCESS);
    }
}
void VulkanDemoApp::destroyFrameBuffer() {
    ...
    free(framebuffers);
    for (int i = 0; i < 5; i++) {
        vkDestroyFramebuffer(device, selfTexFramebuffer[i], nullptr);
    }
    printf("销毁帧缓冲成功！\n");
}
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1501_";
    const char *tp = "../textures/1501_";
    tree1 = LoadUtil::loadFromFile(mp + (string) ("tree.obj"), device, memoryProperties);
    tree2 = LoadUtil::loadFromFile(mp + (string) ("tree01.obj"), device, memoryProperties);
    ...
    sky = new Dashboard2DObject(SkyCloudData::vdata, SkyCloudData::dataByteCount, SkyCloudData::vCount, device, memoryProperties);
    LandData *ld = FileUtil::loadHdtData(tp + (string) ("hd1.bnhdt"));
    landForDraw = new Dashboard2DObject(ld->vData, ld->vCount * 5 * 4, ld->vCount, device, memoryProperties);
}
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    ...
    result = vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsCT->bufferByteCount);
    vkUnmapMemory(device, sqsCT->memUniformBuf);
}
...
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    ...
    vkUpdateDescriptorSets(device, 6, sqsD2D->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex()//将场景绘制到5 幅纹理的方法
{
    const char *tp = "../textures/1501_";
    ...
    for (int i = 0; i < 5; i++) {//循环绘制到5 幅纹理中
        ...
        VulkanDemoApp::flushUniformBufferForToTex();//将一致变量数据送入缓冲
        VulkanDemoApp::flushTexToDesSetForToTex();//更新绘制用描述集
        ...
        MatrixState3D::scale(0.1f, -0.1f, 0.1f);//绕y 轴旋转90°
        landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky(tp + (string) ("grass.bntex"))]));//绘制云天空
        ...
        sky->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                      &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky(tp + (string) ("sky_cloud.bntex"))]));
...
void VulkanDemoApp::drawSceneToScreen()//将5 幅纹理叠加绘制到屏幕的方法
{
    ...
    VulkanDemoApp::flushUniformBufferForToScreen();//将一致变量数据送入缓冲
    VulkanDemoApp::flushTexToDesSetForToScreen();//更新绘制用描述集
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
    MatrixState3D::pushMatrix();//保护现场
...
void VulkanDemoApp::updatePosition() {
    while (!isTreeMoveThreadShouldClose) {
        cz -= SPAN;
        targetz -= SPAN;
        if (cz <= -35) {
            cz = 95;
...
void VulkanDemoApp::drawTrees(int treeIndex, float transX, float transY, float transZ) {
    const char *tp = "../textures/1501_";
    if (treeIndex == 1) {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree1->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("tree.bntex"))]));
        MatrixState3D::popMatrix();
    } else {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree2->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("tree1.bntex"))]));
        MatrixState3D::popMatrix();
    }
}
