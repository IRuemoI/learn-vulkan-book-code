...
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
    ...
    if (left_mouse_down) {
        ...
        VulkanDemoApp::yAngle = (float) (VulkanDemoApp::yAngle + VulkanDemoApp::cursorDeltaX * 180.0 / 600);//y轴旋转角
        VulkanDemoApp::zAngle = (float) (VulkanDemoApp::zAngle + VulkanDemoApp::cursorDeltaY * 180.0 / 600);//z轴旋转角
        ...
        if (left_mouse_down) {
            if (isClick) {//若点击标志为 true
                if (preX < (int) VulkanDemoApp::screenWidth / 2) {//点击位置在屏幕左侧
                    ...
                    VulkanDemoApp::texType = (++VulkanDemoApp::texType % 3);
                }
            }
        }
