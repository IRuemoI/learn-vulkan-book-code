...
int VulkanDemoApp::samplerType = 0;
int VulkanDemoApp::texType = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 30, 0, 0, 0, 0, 1, 0);              // 初始化摄像机
...
{
    float fragmentUniformData[1] = {0.9f};                                                                      //亮度调节系数，与片元着色器中brightFactor对应
    uint8_t *pData;                                                                                             //CPU 访问设备内存时的辅助指针
    VkResult result = vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **) &pData);//将指定设备内存映射为CPU可访问
    assert(result == VK_SUCCESS);                                                                               //检查映射是否成功
    memcpy(pData, fragmentUniformData, sqsCT->bufferByteCount);                                                 //将数据拷贝进设备内存
    vkUnmapMemory(device, sqsCT->memUniformBuf);                                                                //解除内存映射
}
...
void VulkanDemoApp::flushTexToDesSet() const {                                                         //将纹理等数据与描述集关联的方法
    for (int i = 0; i < TextureManager::texNames.size(); i++) {                                        //遍历所有纹理
        ...
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);//写入属性1对应的纹理图像信息
        vkUpdateDescriptorSets(device, 2, sqsCT->writes, 0, nullptr);                                  //更新描述集
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    std::string textureName;               //当前纹理名称
    switch (samplerType) {                 //根据 samplerType 的值分支
        case 0:
            textureName = "../textures/0603_robot0.bntex";
            break;//设置纹理名称 0
        case 1:
            textureName = "../textures/0603_robot1.bntex";
            break;//设置纹理名称 1
        case 2:
            textureName = "../textures/0603_robot2.bntex";
            break;//设置纹理名称 2
        case 3:
            textureName = "../textures/0603_robot3.bntex";
            break;//设置纹理名称 3
    }
    //若 texType 为 0(采用 4×4 纹理坐标范围)
    if (texType == 0) {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));//绘制物体 0
    }
    //若 texType 为 1(采用 4×2 纹理坐标范围)
    else if (texType == 1) {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));//绘制物体 1
    }
    //若 texType 为 2(采用 1×1 纹理坐标范围)
    else if (texType == 2) {
        texTri2->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));//绘制物体 2
    }
...
void VulkanDemoApp::initPipeline() {
    sqsCT = new ShaderQueueSuit_CommonTex(&device, renderPass, memoryProperties);// 创建封装了渲染管线相关的对象
}
