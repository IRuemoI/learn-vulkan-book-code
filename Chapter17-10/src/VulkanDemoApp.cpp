...
VkImageView VulkanDemoApp::colorImageView[SELF_COLOR_COUNT];
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    treeForDraw = LoadUtil::loadFromFile("../models/1710_tree.obj", device, memoryProperties);
    CHForDraw = LoadUtil::loadFromFile("../models/1710_ch_t.obj", device, memoryProperties);
    carForDraw = LoadUtil::loadFromFile("../models/1710_car.obj", device, memoryProperties);
    floorForDraw = LoadUtil::loadFromFile("../models/1710_pm.obj", device, memoryProperties);
}
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsL->memUniformBuf, 0, sqsL->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsL->bufferByteCount);
    vkUnmapMemory(device, sqsL->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSet() {
    ...
    vkUpdateDescriptorSets(device, 4, sqsL->writes, 0, nullptr);
}
void VulkanDemoApp::drawObject() {
    ...
    CHForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,//绘制茶壶
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1710_ghxp.bntex")]));
    ...
    treeForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                          &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1710_tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87, 0, 50);
    carForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                         &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1710_car.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    floorForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1710_floor.bntex")]));
    MatrixState3D::popMatrix();
    vkCmdNextSubpass(cmdBuffer, VK_SUBPASS_CONTENTS_INLINE);//跳转到下一个子通道
    vkCmdBindPipeline(cmdBuffer//绑定第二个子通道的管线
                      ,
...
void VulkanDemoApp::initPipeline() {
    sqsCTL = new ShaderQueueSuit_PreProcess(&device, renderPass, memoryProperties);
    sqsL = new ShaderQueueSuit_Light(&device, renderPass, memoryProperties);
}
...
void VulkanDemoApp::create_vulkan_SelfColorBuffer() {
    VkFormat colorFormat[4] = {VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_R32G32B32A32_SFLOAT, VK_FORMAT_R32G32B32A32_SFLOAT};
    for (int i = 0; i < 3; i++) {
