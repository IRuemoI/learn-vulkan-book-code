...
void ShaderQueueSuit_Cubemap::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    bufferByteCount = 4 * sizeof(float);
...
void ShaderQueueSuit_Cubemap::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_Cubemap::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_Cubemap::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_Cubemap::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_Cubemap::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/cubemap.vert";
    const char *fs = "./shaders/cubemap.frag";
...
void ShaderQueueSuit_Cubemap::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_Cubemap::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_Cubemap::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_Cubemap::ShaderQueueSuit_Cubemap(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
