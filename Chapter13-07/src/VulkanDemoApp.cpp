...
string guangStr[] = {"../textures/1307_flare1.bntex", "../textures/1307_flare2.bntex", "../textures/1307_flare3.bntex"};
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texForDraw = new ObjObject(RectData::vData, RectData::dataByteCount, RectData::vCount, device, memoryProperties);
    FlatData::genVertexData();
    guangForDraw = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryProperties);
    flare = new Flare();
}
// 销毁绘制用物体
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsCube->memUniformBuf, 0, sqsCube->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCube->bufferByteCount);
    vkUnmapMemory(device, sqsCube->memUniformBuf);
...
void VulkanDemoApp::flushTexToDesSet() const {// 更新绘制用描述集的方法
    for (int i = 0; i < TextureManager::texNamesForSkyBox.size(); i++) {
        ...
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForSkyBox[i]]);
    ...
    for (int i = 0; i < TextureManager::texNamesForGuang.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCube->writes, 0, nullptr);
    }
}


void VulkanDemoApp::drawSky() {
    const float tzz = 0.4f;
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 0, Constant::UNIT_SIZE - tzz);
    MatrixState3D::rotate(180, 0, 1, 0);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_front.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 0, -Constant::UNIT_SIZE + tzz);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_back.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(Constant::UNIT_SIZE - tzz, 0, 0);
    MatrixState3D::rotate(90, 0, 1, 0);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_right.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-Constant::UNIT_SIZE + tzz, 0, 0);
    MatrixState3D::rotate(-90, 0, 1, 0);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_left.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, Constant::UNIT_SIZE - tzz, 0);
    MatrixState3D::rotate(90, -1, 0, 0);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_up.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, -Constant::UNIT_SIZE + tzz, 0);
    MatrixState3D::rotate(90, 1, 0, 0);
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox("../textures/1307_skycubemap_down.bntex")]));
    MatrixState3D::popMatrix();
}
...
void VulkanDemoApp::drawObject() {
    ...
    if (lpx < Constant::RATIO && lpy < 1) {//若光源投影位置在屏幕可视范围内
        ...
        for (auto ss: flare->sFl) {                                                                                                                                                            //遍历光晕元素列表
            ...
            guangForDraw->drawSelf(cmdBuffer, sqsCube->pipelineLayout, sqsCube->pipeline, &(sqsCube->descSet[TextureManager::getVkDescriptorSetIndexForGuang(guangStr[ss->idIn])]), ss->color);//绘制光晕元素
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsCube = new ShaderQueueSuit_Cubemap(&device, renderPass, memoryProperties);
}
