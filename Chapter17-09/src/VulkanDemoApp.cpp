...
std::vector<PlanePosition *> VulkanDemoApp::planeVector;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.9", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    planeVector.push_back(planePosition0);
    planeVector.push_back(planePosition1);
    planeVector.push_back(planePosition2);
    planeVector.push_back(planePosition3);
}
            ...
            switch (planeP->id) {
                case 0:
                    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("plane01.bntex"))]));
                    break;
                case 1:
                    objObject1->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                         &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("plane02.bntex"))]));
                    break;
                case 2:
                    objObject2->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                         &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("plane03.bntex"))]));
                    break;
                case 3:
                    objObject3->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                         &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) ("plane04.bntex"))]));
                    break;
            }
