...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 11.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    LYobjDraw1 = LoadUtil::loadFromFile("../models/1107_zd.obj", device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(4000, 4000, 30, 0, 0, 0, 0, 1, 0);
...
void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    ...
    float uniformData[1] = {uFatFactor};
    uint8_t *pData;
...
void VulkanDemoApp::drawObject() {
    ...
    LYobjDraw1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1107_zd_bg.bntex")]));
