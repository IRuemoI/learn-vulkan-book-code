...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObject = LoadUtil::loadFromFile("../models/1305_rect.obj", device, memoryProperties);
    objObjectUt = LoadUtil::loadFromFileUt("../models/1305_rect.obj", device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1305_ghxp.bntex")]));
    ...
    MatrixState3D::rotate(yAngle, 0, 1, 0);
    MatrixState3D::rotate(xAngle, 1, 0, 0);
    objObjectUt->drawSelf(cmdBuffer, sqsUt->pipelineLayout, sqsUt->pipeline,
                          &(sqsUt->descSet[0]));
    MatrixState3D::popMatrix();
