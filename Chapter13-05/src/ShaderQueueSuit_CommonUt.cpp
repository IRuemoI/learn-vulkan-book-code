...
#include <assert.h>
void ShaderQueueSuit_CommonUt::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_CommonUt::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_CommonUt::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonUt::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_CommonUt::initDescriptorSet(VkDevice &device) {
    VkDescriptorPoolSize type_count[3];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::texNames.size();
    //type_count[0].descriptorCount = TextureManager::texNames.size() / 2;//TODO:不知道书中为啥这么写 会报错
...
void ShaderQueueSuit_CommonUt::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/shaderUT.vert";
    const char *fs = "./shaders/shaderUT.frag";
...
void ShaderQueueSuit_CommonUt::destroyShaderModule(VkDevice &device) {
    vkDestroyShaderModule(device, shaderStages[0].module, nullptr);
    vkDestroyShaderModule(device, shaderStages[1].module, nullptr);
}
void ShaderQueueSuit_CommonUt::initVertexAttributeInfo() {
...
void ShaderQueueSuit_CommonUt::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_CommonUt::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_CommonUt::ShaderQueueSuit_CommonUt(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
ShaderQueueSuit_CommonUt::~ShaderQueueSuit_CommonUt() {
