...
bool VulkanDemoApp::isAnimationThreadShouldClose = false;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 16.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::mainLoop() {
    ...
    animationThread = std::thread(&VulkanDemoApp::updateBNGGDH, this);
    ...
    isAnimationThreadShouldClose = true;
...
void VulkanDemoApp::createDrawableObject() {
    const char *bp = "./Animation/";
    const char *tp = "../textures/1604_";
    bnModelA = new BNModel(bp + (string) ("a.bnggdh"), tp + (string) ("a.bntex"), 0.0014f, device, memoryProperties);
    bnModelB = new BNModel(bp + (string) ("b.bnggdh"), tp + (string) ("b.bntex"), 0.002f, device, memoryProperties);
}
...
void VulkanDemoApp::updateBNGGDH() {       //用于更新骨骼动画的方法
    while (!isAnimationThreadShouldClose) {//若循环绘制标志为true
