...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 12.6", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    ...
    float fragmentUniformData[1] = {factor};
    uint8_t *pData;
...
void VulkanDemoApp::initPipeline() {
    sqsCT = new ShaderQueueSuit_CommonTexDouble(&device, renderPass, memoryProperties);
}
