...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObjectCH = LoadUtil::loadFromFile("../models/1402_ch.obj", device, memoryProperties);
    objObjectCFT = LoadUtil::loadFromFile("../models/1402_cft.obj", device, memoryProperties);
    objObjectQT = LoadUtil::loadFromFile("../models/1402_qt.obj", device, memoryProperties);
    objObjectYH = LoadUtil::loadFromFile("../models/1402_yh.obj", device, memoryProperties);
    objObjectPM = LoadUtil::loadFromFile("../models/1402_pm.obj", device, memoryProperties);
    printf("模型加载成功  \n");
}
...
void VulkanDemoApp::flushTexToDesSet() {
    ...
    vkUpdateDescriptorSets(device, 1, sqsCT->writes, 0, nullptr);
}

void VulkanDemoApp::drawThings(float isShadow)//绘制平面上各个物体的方法
{
...
void VulkanDemoApp::drawObject() {//执行绘制的方法
    FPSUtil::calFPS();
    FPSUtil::before();
    lightAzimuth += 1;
    lightAzimuth %= 360;
