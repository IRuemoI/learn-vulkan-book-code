...
void main() {
    ...
    float tzH=sin(currAngleH)*0.3;//X方向波浪对应的Z坐标
    ...
    float currAngleZ=myBufferVals.uStartAngle+3.14159265/3.0+((pos.y-startY)/uHeightSpan)*angleSpanZ;
    float tzZ=sin(currAngleZ)*0.3;//Y方向波浪对应的Z坐标

    outTexCoor = inTexCoor;
    gl_Position = myConstantVals.mvp * vec4(pos.x, pos.y, tzH+tzZ, 1.0);
}
