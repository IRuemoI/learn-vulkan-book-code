...
float VulkanDemoApp::uWidthSpan = 12.0f;
float VulkanDemoApp::uStartAngle = 0.0f;
int VulkanDemoApp::isSwap = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 11.1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texTri = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 15, 0, 0, 0, 0, 1, 0);
...
void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    ...
    result = vkMapMemory(device, sqsXY->memUniformBuf, 0, sqsXY->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsXY->bufferByteCount);
    vkUnmapMemory(device, sqsXY->memUniformBuf);
    result = vkMapMemory(device, sqsXie->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsXY->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
...
void VulkanDemoApp::drawObject() {
    ...
    if (isSwap == 0) {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    } else if (isSwap == 1) {
        texTri->drawSelf(cmdBuffer, sqsXY->pipelineLayout, sqsXY->pipeline, &(sqsXY->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    } else if (isSwap == 2) {
        texTri->drawSelf(cmdBuffer, sqsXie->pipelineLayout, sqsXie->pipeline, &(sqsXie->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    }
...
void VulkanDemoApp::initPipeline() {
    sqsCT = new ShaderQueueSuit_CommonTex_X(&device, renderPass, memoryProperties);
    sqsXY = new ShaderQueueSuit_CommonTex_XY(&device, renderPass, memoryProperties);
    sqsXie = new ShaderQueueSuit_CommonTex_Xie(&device, renderPass, memoryProperties);
}
