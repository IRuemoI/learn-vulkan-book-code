...
bool mouseLeftButtonHold = false;
void cursor_pos_callback(GLFWwindow *window, double xpos, double ypos) {
    ...
    if (left_mouse_down) {
        ...
        if (abs((int) VulkanDemoApp::cursorDeltaX) > 2 || abs((int) VulkanDemoApp::cursorDeltaY) > 2) {
            mouseLeftButtonHold = true;
        }
        VulkanDemoApp::yAngle = (float) (VulkanDemoApp::yAngle + VulkanDemoApp::cursorDeltaX * 180.0 / 600);
        VulkanDemoApp::zAngle = (float) (VulkanDemoApp::zAngle + VulkanDemoApp::cursorDeltaY * 180.0 / 600);
    }
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (!mouseLeftButtonHold) {
            VulkanDemoApp::isSwap = (++VulkanDemoApp::isSwap) % 3;
            printf("切换到模式:%d\n", VulkanDemoApp::isSwap);
            mouseLeftButtonHold = false;
        }
        printf("鼠标松开\n");
    }
}
