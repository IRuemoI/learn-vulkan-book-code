...
#include <assert.h>
void ShaderQueueSuit_CommonDepth::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_CommonDepth::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_CommonDepth::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonDepth::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_CommonDepth::initDescriptorSet(VkDevice &device) {
...
void ShaderQueueSuit_CommonDepth::createShaderModule(VkDevice &device) {
    std::string vertStr = FileUtil::loadAssetStr("./shaders/commonDepth.vert");
    std::string fragStr = FileUtil::loadAssetStr("./shaders/commonDepth.frag");
...
void ShaderQueueSuit_CommonDepth::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_CommonDepth::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_CommonDepth::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_CommonDepth::ShaderQueueSuit_CommonDepth(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
