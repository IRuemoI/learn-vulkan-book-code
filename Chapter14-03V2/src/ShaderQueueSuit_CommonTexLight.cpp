...
void ShaderQueueSuit_CommonTexLight::create_uniform_buffer_vert(VkDevice &device,
    ...
    buf_info.flags = 0;
    VkResult result = vkCreateBuffer(device, &buf_info, nullptr, &uniformBufVert);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(device, uniformBufVert, &mem_reqs);
    ...
    result = vkAllocateMemory(device, &alloc_info, nullptr, &memUniformBufVert);
    assert(result == VK_SUCCESS);
    result = vkBindBufferMemory(device, uniformBufVert, memUniformBufVert, 0);
    assert(result == VK_SUCCESS);
...
void ShaderQueueSuit_CommonTexLight::destroy_uniform_buffer_vert(VkDevice &device) {
    vkDestroyBuffer(device, uniformBufVert, nullptr);
    vkFreeMemory(device, memUniformBufVert, nullptr);
}
...
void ShaderQueueSuit_CommonTexLight::createShaderModule(VkDevice &device) {
    std::string vertStr = FileUtil::loadAssetStr("./shaders/commonTexLight.vert");
    std::string fragStr = FileUtil::loadAssetStr("./shaders/commonTexLight.frag");
...
void ShaderQueueSuit_CommonTexLight::create_uniform_buffer_frag(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
