...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.3V2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::destroyVulkanDepthBuffer()// 销毁深度缓冲相关
{
...
void VulkanDemoApp::createDrawableObject() {
    treeForDraw = LoadUtil::loadFromFile("../models/1403_tree.obj", device, memoryProperties);
    CHForDraw = LoadUtil::loadFromFile("../models/1403_ch_t.obj", device, memoryProperties);
    carForDraw = LoadUtil::loadFromFile("../models/1403_car.obj", device, memoryProperties);
    pmForDraw = LoadUtil::loadFromFile("../models/1403_pm.obj", device, memoryProperties);
...
void VulkanDemoApp::flushUniformBufferForToTex() {
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsCD->memUniformBuf, 0, sqsCD->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCD->bufferByteCount);
    vkUnmapMemory(device, sqsCD->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSetForToTex() {
    sqsCD->writes[0].dstSet = sqsCD->descSet[0];
    vkUpdateDescriptorSets(device, 1, sqsCD->writes, 0, nullptr);
}

void VulkanDemoApp::drawSceneToTex() {                               //将场景绘制到纹理的方法
    ...
    LightManager::getTYJZ(screenWidth * 1.0f / screenHeight);        //获取投影与观察组合矩阵的方法
    flushUniformBufferForToTex();                                    //将一致变量缓冲送入渲染管线
    flushTexToDesSetForToTex();                                      //更新绘制用描述集
    ...
    //绘制茶壶
    CHForDraw->drawSelf(cmdBuffer, sqsCD->pipelineLayout, sqsCD->pipeline, &(sqsCD->descSet[0]), true);
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-87, 0, 50);
    treeForDraw->drawSelf(cmdBuffer, sqsCD->pipelineLayout, sqsCD->pipeline, &(sqsCD->descSet[0]), true);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87, 0, 50);
    carForDraw->drawSelf(cmdBuffer, sqsCD->pipelineLayout, sqsCD->pipeline, &(sqsCD->descSet[0]), true);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    pmForDraw->drawSelf(cmdBuffer, sqsCD->pipelineLayout, sqsCD->pipeline, &(sqsCD->descSet[0]), true);
    MatrixState3D::popMatrix();
    vkCmdEndRenderPass(cmdBuffer);
    result = vkEndCommandBuffer(cmdBuffer);//结束渲染通道
...
void VulkanDemoApp::drawSceneToScreen() {
    ...
    MatrixState3D::scale(3, 3, 3);       //进行缩放变换
    //绘制茶壶
    CHForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1403_ghxp.bntex")]), false);
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-87, 0, 50);
    treeForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1403_tree.bntex")]), false);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87, 0, 50);
    carForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1403_car.bntex")]), false);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    pmForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1403_floor.bntex")]), false);
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    sqsCD = new ShaderQueueSuit_CommonDepth(&device, renderPass, memoryProperties);
...
void VulkanDemoApp::destroyVulkanSelfColorBuffer() {
...
void VulkanDemoApp::flushUniformBufferForToScreen() {
    float vertexUniformData[20] =
            {
                    CameraUtil::camera9Para[0],
                    CameraUtil::camera9Para[1],
                    CameraUtil::camera9Para[2],
                    1.0,
                    LightManager::lx,
                    LightManager::ly,
                    LightManager::lz,
                    1.0,
                    LightManager::lightAmbientR,
                    LightManager::lightAmbientG,
                    LightManager::lightAmbientB,
                    LightManager::lightAmbientA,
                    LightManager::lightDiffuseR,
                    LightManager::lightDiffuseG,
                    LightManager::lightDiffuseB,
                    LightManager::lightDiffuseA,
                    LightManager::lightSpecularR,
                    LightManager::lightSpecularG,
                    LightManager::lightSpecularB,
                    LightManager::lightSpecularA,
            };
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsCTL->memUniformBufVert, 0, sqsCTL->bufferByteCountVert, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTL->bufferByteCountVert);
    vkUnmapMemory(device, sqsCTL->memUniformBufVert);
    float vertexUniformDataFrag[18] =
            {
                    ...
                    LIGNT_CAMERA_NEAR, LIGNT_CAMERA_FAR};
    result = vkMapMemory(device, sqsCTL->memUniformBufFrag, 0, sqsCTL->bufferByteCountFrag, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformDataFrag, sqsCTL->bufferByteCountFrag);
    vkUnmapMemory(device, sqsCTL->memUniformBufFrag);
}
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 4, sqsCTL->writes, 0, nullptr);
    }
}