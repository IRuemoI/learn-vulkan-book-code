...
void ShaderQueueSuit_Common::createShaderModule(VkDevice &device) {
    SpvData spvVertData = FileUtil::loadSPV("./shaders/commonTexLight.vert.spv");//加载顶点SPV
    SpvData spvFragData = FileUtil::loadSPV("./shaders/commonTexLight.frag.spv");//加载片元SPV
...
void ShaderQueueSuit_Common::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
    ...
    memset(dynamicStateEnables, 0,
           sizeof dynamicStateEnables);// 设置所有标志为false
