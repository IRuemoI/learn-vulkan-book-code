...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 2.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initVulkanInstance() {
    ...
    if (!exceptedLayerNames.empty()) {                        //若能够启动的验证Layer数量大于0
        ValidateUtil::createDebugReportCallbackSelf(instance);//创建调试报告回调
    }
}
...
void VulkanDemoApp::createDrawableObject() {
    ...
    triForDraw = new DrawableObjectCommonLight(TriangleData::vdata, TriangleData::dataByteCount, TriangleData::vCount,
                                               device, memoryProperties);// 创建绘制用三色三角形对象
}
...
void VulkanDemoApp::drawObject() {
    ...
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin,
                         VK_SUBPASS_CONTENTS_INLINE);// 启动渲染通道
