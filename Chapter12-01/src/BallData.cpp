...
void BallData::genBallData(float angleSpan) {
    ...
    for (float vAngle = -90; vAngle < 90; vAngle = vAngle + angleSpan) {
        for (float hAngle = 0; hAngle <= 360; hAngle = hAngle + angleSpan) {
            ...
            float long2 = hAngle + angleSpan;
            float lat2 = vAngle + angleSpan;
            ...
            alVertix.push_back(y3);
            alVertix.push_back(z3);
            alLongLat.push_back(long1);
            alLongLat.push_back(lat1);
            alLongLat.push_back(long3);
            alLongLat.push_back(lat3);
            alLongLat.push_back(long0);
            alLongLat.push_back(lat0);
            alLongLat.push_back(long1);
            alLongLat.push_back(lat1);
            alLongLat.push_back(long2);
            alLongLat.push_back(lat2);
            alLongLat.push_back(long3);
            alLongLat.push_back(lat3);
        }
    }
