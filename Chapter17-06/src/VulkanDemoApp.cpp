...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
VulkanDemoApp::get_required_extensions() {
    ...
    std::vector<const char *> extensions(glfwExtensions, glfwExtensions + glfwExtensionCount);// 申请glfwExtensionCount个用于存储扩展名称的向量单位空间
    return extensions;
}
...
void VulkanDemoApp::createVulkanSwapChain() {
    ...
    result = vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[USED_GPU_INDEX], surface, &formatCount, surfFormats);// 获取支持的格式信息
...
void VulkanDemoApp::flushTexToDesSetForBD() {//更新计算波动描述集的方法
    ...
    vkUpdateDescriptorSets(device, 3, sqsBD->writes, 0, nullptr);//更新描述集
}

void VulkanDemoApp::flushTexToDesSetForNormal() {//更新计算法向量用描述集
    ...
    vkUpdateDescriptorSets(device, 1, sqsNormal->writes, 0, nullptr);//更新描述集
}
...
void VulkanDemoApp::drawTask() {
    ...
    waterForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline, &(sqsSTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1706_haimian.bntex")]));
...
void VulkanDemoApp::calTaskBD() {
    ...
    waterForDraw->calSelfBD(cmdBuffer, sqsBD->pipelineLayout, sqsBD->pipeline, &(sqsBD->descSet[0]));
    result = vkEndCommandBuffer(cmdBuffer);
...
void VulkanDemoApp::calTaskNormal() {
    ...
    waterForDraw->calSelfNormal(cmdBuffer, sqsNormal->pipelineLayout, sqsNormal->pipeline, &(sqsNormal->descSet[0]));
    result = vkEndCommandBuffer(cmdBuffer);
    result = vkQueueSubmit(queueGraphics, 1, &submit_info[1], taskFinishFence);
    do {
        result = vkWaitForFences(device, 1, &taskFinishFence, VK_TRUE, FENCE_TIMEOUT);
