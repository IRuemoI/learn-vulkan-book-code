...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 12.13", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    LandData *ld = FileUtil::loadHdtData("./hdt/land.bnhdt");
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    float uniformData[5] = {CameraUtil::cx, CameraUtil::cy, CameraUtil::cz, 0, uFactor};
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsDT->memUniformBuf, 0, sqsDT->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsDT->bufferByteCount);
    vkUnmapMemory(device, sqsDT->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSet() const {
    for (int i = 0; i < TextureManager::texNames.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 3, sqsDT->writes, 0, nullptr);
    }
}
