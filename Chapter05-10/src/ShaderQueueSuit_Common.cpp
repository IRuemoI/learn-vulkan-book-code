...
void ShaderQueueSuit_Common::initVertexAttributeInfo() {
    ...
    vertexAttributes[0].binding = 0;//顶点坐标输入属性的绑定点
    vertexAttributes[0].location = 0;//顶点坐标输入属性的位置索引
    vertexAttributes[0].format = VK_FORMAT_R32G32B32_SFLOAT;//顶点坐标输入属性的数据格式
    vertexAttributes[0].offset = 0;//顶点坐标输入属性的偏移量

    vertexAttributes[1].binding = 0;//法向量输入属性的绑定点
    vertexAttributes[1].location = 1;//法向量输入属性的位置索引
    vertexAttributes[1].format = VK_FORMAT_R32G32B32_SFLOAT;//法向量输入属性的数据格式
    vertexAttributes[1].offset = 12;//法向量输入属性的偏移量
}
