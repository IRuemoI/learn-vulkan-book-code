...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 5.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createRenderPass() {
    ...
    clear_values[0].color.float32[0] = 0;     // 帧缓冲清除用R分量值
    clear_values[0].color.float32[1] = 0;     // 帧缓冲清除用G分量值
    clear_values[0].color.float32[2] = 0;     // 帧缓冲清除用B分量值
    clear_values[0].color.float32[3] = 0;     // 帧缓冲清除用A分量值
...
void VulkanDemoApp::createDrawableObject() {
    FlatData::genData();
    objForDraw = new DrawableObjectCommonLight(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryProperties);//创建绘制用球
}
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 30.0f, 0, 0, 0, 0, 1, 0);            // 初始化摄像机
    ...
    LightManager::setLightPosition(0, 0, -14.5f);                       //设置光源位置
