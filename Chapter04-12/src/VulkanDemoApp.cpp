...
float VulkanDemoApp::zAngle = 20;
float VulkanDemoApp::yAngle = 90;
int VulkanDemoApp::ProjectPara = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.12", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    cube1ForDraw = new Cube(device, memoryProperties, ColorRect::vdataG, ColorRect::UNIT_SIZEG);
    cube2ForDraw = new Cube(device, memoryProperties, ColorRect::vdataY, ColorRect::UNIT_SIZEY);
}
...
void VulkanDemoApp::initMatrix() {
    ...
    if (ProjectPara) { NEARt = 800.0f; }                                              //较大的NEAR 值
        ...
        NEARt = 1.0f;//较小的NEAR 值
    }
    MatrixState3D::setProjectFrustum(-NEARt * ratio * 0.25f, NEARt * ratio * 0.25f, -NEARt * 0.25f, NEARt * 0.25f, NEARt, 10000.0f);//设置透视投影参数
}
