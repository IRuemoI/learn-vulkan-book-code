...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    homeObjForDraw = LoadUtil::loadFromFile("../models/1310_skybox.obj", device, memoryProperties);
    bedObjForDraw = LoadUtil::loadFromFile("../models/1310_chuang.obj", device, memoryProperties);
    dengObjForDraw = LoadUtil::loadFromFile("../models/1310_deng.obj", device, memoryProperties);
    book0ObjForDraw = LoadUtil::loadFromFile("../models/1310_shu0.obj", device, memoryProperties);
    book1ObjForDraw = LoadUtil::loadFromFile("../models/1310_shu1.obj", device, memoryProperties);
    chairObjForDraw = LoadUtil::loadFromFile("../models/1310_yizi0.obj", device, memoryProperties);
    borderObjForDraw = LoadUtil::loadFromFile("../models/1310_mirror.obj", device, memoryProperties);
    /*创建镜面*/
    float UNIT_SIZE = 15.0f;
    float *vdataIn = new float[18]{
            -UNIT_SIZE, -UNIT_SIZE, 0, UNIT_SIZE, UNIT_SIZE, 0, -UNIT_SIZE, UNIT_SIZE, 0,
            -UNIT_SIZE, -UNIT_SIZE, 0, UNIT_SIZE, -UNIT_SIZE, 0, UNIT_SIZE, UNIT_SIZE, 0};
    tr = new TextureRect(vdataIn, 6 * 3 * sizeof(float), 6, device, memoryProperties);
    printf("模型加载成功  \n");
}
...
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    ...
    vkUpdateDescriptorSets(device, 2, sqsM->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex() {                               //将场景镜像绘制到纹理的方法
    ...
    flushUniformBufferForToTex();                                    //将一致变量数据送入缓冲
...
void VulkanDemoApp::drawSceneToScreen() { //将场景绘制到屏幕的方法
    ...
    vkResetCommandBuffer(cmdBuffer, 0);                     //重置命令缓冲到初始状态
    result = vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);//启动命令缓冲
    flushUniformBufferForToScreen();                        //将一致变量数据送入缓冲
    flushTexToDesSetForToScreen();                          //更新绘制用描述集
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
    drawThings();                                                //绘制除镜面外的其它物体
    MatrixState3D::pushMatrix();                                 //保护现场
    MatrixState3D::translate(2.2f, 12.0f, CameraUtil::tz - 2.0f);//执行平移变换

    borderObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_mirror.bntex")]));//绘制镜子边框
...
void VulkanDemoApp::drawThings() {
    ...
    homeObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_skybox.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(40, 0, -12);
    MatrixState3D::rotate(180, 0, 1, 0);
    dengObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_deng.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(14, -7, -18);
    book1ObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_shu1.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(22, 0, 8);
    MatrixState3D::rotate(180, 0, 1, 0);
    bedObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_chuang.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-40, 30, 10);
    book0ObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_shu0.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    ...
    chairObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_yizi.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
