...
int VulkanDemoApp::shotBytesCount;
...
void VulkanDemoApp::create_vulkan_SelfColorBuffer() {
    ...
    buf_info.flags = 0;
    result = vkCreateBuffer(device, &buf_info, nullptr, &shotBuffer);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqsShot;
    vkGetBufferMemoryRequirements(device, shotBuffer, &mem_reqsShot);
    ...
    flag = memoryTypeFromProperties(memoryProperties, mem_reqsShot.memoryTypeBits, requirements_mask, &alloc_info.memoryTypeIndex);
    if (flag) {
        printf("\nShot确定内存类型成功 类型索引为%d\n", alloc_info.memoryTypeIndex);
    } else {
        printf("\nShot确定内存类型失败!\n");
    }
    result = vkAllocateMemory(device, &alloc_info, nullptr, &shotMem);
    assert(result == VK_SUCCESS);
    result = vkBindBufferMemory(device, shotBuffer, shotMem, 0);
    assert(result == VK_SUCCESS);
}
...
void VulkanDemoApp::createFrameBuffer() {
    ...
    VkResult result = vkCreateFramebuffer(device,
                                          &fb_info, nullptr, &selfTexFramebuffer);
    assert(result == VK_SUCCESS);
}
...
void VulkanDemoApp::drawSceneToTex() {
    ...
    MatrixState3D::scale(3.0f, 3.0f, 3.0f);
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1803_SpaceStation512.bntex")]));
