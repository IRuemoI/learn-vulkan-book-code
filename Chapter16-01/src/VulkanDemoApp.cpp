...
ShaderQueueSuit_Common *VulkanDemoApp::sqsCL;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 16.1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createVulkanDevices() {
    ...
    VkResult result = vkCreateDevice(gpus[USED_GPU_INDEX], &deviceInfo, nullptr,
                                     &device);// 创建逻辑设备
...
void VulkanDemoApp::createDrawableObject() {
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_body.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_head.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_left_top.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_left_bottom.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_right_top.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_right_bottom.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_right_leg_top.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_right_leg_bottom.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_left_leg_top.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_left_leg_bottom.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_left_foot.obj", device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile("../models/1601_right_foot.obj", device, memoryProperties));
    robot = new Robot(robotObj);
    plane = LoadUtil::loadFromFile("../models/1601_floor.obj", device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    robot->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1601_arm.bntex")]));
    MatrixState3D::popMatrix();
    /*
		MatrixState3D::pushMatrix();
		MatrixState3D::scale(0.1, 0.1, 0.1);
		plane->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
			&(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1601_wenli.bntex")]));
		MatrixState3D::popMatrix();
*/
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    sqsCL = new ShaderQueueSuit_Common(&device, renderPass, memoryProperties);// 创建封装了渲染管线相关的对象
}
