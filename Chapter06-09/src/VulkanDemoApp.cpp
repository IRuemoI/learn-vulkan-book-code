...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.9", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {//创建绘制用物体的方法
    ...
    texBall = new TexDrawableObject(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 70, 0, 0, 0, 0, 1, 0);              // 初始化摄像机
    MatrixState3D::setInitStack();                                     // 初始化基本变换矩阵
    float ratio = (float) screenWidth / (float) screenHeight;          // 求屏幕长宽比
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 4.0f, 1000);// 设置投影参数
}
...
void VulkanDemoApp::drawObject() {
    ...
    texBall->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0609_boardRed.bn3dtex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(15, 0, 0);
    MatrixState3D::rotate(yAngle, 0, 1, 0);
    MatrixState3D::rotate(zAngle, 0, 0, 1);
    texBall->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0609_boardGreen.bn3dtex")]));
