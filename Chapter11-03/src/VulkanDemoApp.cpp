...
VkCommandBuffer VulkanDemoApp::cmdBuffer;

uint32_t VulkanDemoApp::screenWidth; // 屏幕宽度
uint32_t VulkanDemoApp::screenHeight;// 屏幕高度

ShaderQueueSuit_CommonTex *VulkanDemoApp::sqsCT;
ShaderQueueSuit_CommonTex_Xie *VulkanDemoApp::sqsXie;
ShaderQueueSuit_CommonTexDouble *VulkanDemoApp::sqsDou;
ShaderQueueSuit_CommonTexTrunk *VulkanDemoApp::sqsTrunk;
ShaderQueueSuit_CommonTexBlend *VulkanDemoApp::sqsBlend;
TexDrawableObject *VulkanDemoApp::landForDraw;
TexDrawableObject *VulkanDemoApp::waterForDraw;
TexDrawableObject *VulkanDemoApp::skyForDraw;
TreeControl *VulkanDemoApp::treeControl;
TexDrawableObject *VulkanDemoApp::texTri;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 11.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texTri = new TexDrawableObject(ObjectData::vdata, ObjectData::dataByteCount, ObjectData::vCount, device, memoryProperties);

    FlatData::genVertexData();                                                                                                        //初始化水面数据
    waterForDraw = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryProperties);       //水面绘制者
    ...
    for (int i = 0; i < 4; i++) {//循环创建树干
        treeControl->trunkVector.push_back(
                new TexTrunkDrawableObject(TreeTrunkData::vdata, TreeTrunkData::dataByteCount, TreeTrunkData::vCount, device, memoryProperties, treePostion[i][0], treePostion[i][1], treePostion[i][2]));//创建树干
    }
    for (int i = 0; i < treeControl->trunkVector.size(); i++) {//循环创建树叶
        for (int j = 0; j < 6; j++) {
            treeControl->leafVector.push_back(
                    new TexLeafDrawableObject(TreeLeaves::vdata, TreeLeaves::dataByteCount, TreeLeaves::vCount, device, memoryProperties, treePostion[i][0], treePostion[i][1], treePostion[i][2], j * 60, j));
        }
    }
    LandData *ld = FileUtil::loadHdtData("./hdt/land1.bnhdt");                                               //初始化灰度图
    landForDraw = new TexDrawableObject(ld->vData, ld->vCount * 5 * 4, ld->vCount, device, memoryProperties);//山体绘制者

    SkyBallData::genVertexData();
    skyForDraw = new TexDrawableObject(SkyBallData::vdata, SkyBallData::dataByteCount, SkyBallData::vCount, device, memoryProperties);//天空盒
}
...
void VulkanDemoApp::flushUniformBuffer()// 将当前帧相关数据送入一致变量缓冲
{
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsXie->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformWaterData, sqsXie->bufferByteCount);
    vkUnmapMemory(device, sqsXie->memUniformBuf);
    ...
    result = vkMapMemory(device, sqsTrunk->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformTreeData, sqsTrunk->bufferByteCount);
    vkUnmapMemory(device, sqsTrunk->memUniformBuf);
}
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::waterNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsXie->writes, 0, nullptr);
    }
    //加载双纹理
    for (int i = 0; i < TextureManager::doubleNames.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 3, sqsDou->writes, 0, nullptr);
    }
    //送入树叶纹理
    sqsBlend->writes[0].dstSet = sqsBlend->descSet[0];
    sqsBlend->writes[1].dstSet = sqsBlend->descSet[0];
    sqsBlend->writes[1].pImageInfo = &(TextureManager::texImageInfoList["../textures/1103_leaf.bntex"]);
    vkUpdateDescriptorSets(device, 2, sqsBlend->writes, 0, nullptr);
    //送入树干纹理
    sqsTrunk->writes[0].dstSet = sqsTrunk->descSet[0];
    sqsTrunk->writes[1].dstSet = sqsTrunk->descSet[0];
    sqsTrunk->writes[1].pImageInfo = &(TextureManager::texImageInfoList["../textures/1103_treetrunk.bntex"]);
    vkUpdateDescriptorSets(device, 2, sqsTrunk->writes, 0, nullptr);
}
void VulkanDemoApp::drawObject() {
    ...
    waterForDraw->drawSelf(cmdBuffer, sqsXie->pipelineLayout, sqsXie->pipeline, &(sqsXie->descSet[TextureManager::getVkDescriptorWaterSetIndex("../textures/1103_water.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();//绘制天空穹
    skyForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1103_sky.bntex")]));
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsXie = new ShaderQueueSuit_CommonTex_Xie(&device, renderPass, memoryProperties);   //创建水面管线
    sqsDou = new ShaderQueueSuit_CommonTexDouble(&device, renderPass, memoryProperties); //双纹理管线
    sqsBlend = new ShaderQueueSuit_CommonTexBlend(&device, renderPass, memoryProperties);//创建混合管线
    sqsTrunk = new ShaderQueueSuit_CommonTexTrunk(&device, renderPass, memoryProperties);//创建树管线
}
