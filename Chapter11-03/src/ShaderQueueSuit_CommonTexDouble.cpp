...
void ShaderQueueSuit_CommonTexDouble::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_CommonTexDouble::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_CommonTexDouble::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonTexDouble::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_CommonTexDouble::initDescriptorSet(VkDevice &device) {
    ...
    descriptor_pool.maxSets = TextureManager::doubleNames.size();
    ...
    for (int i = 0; i < TextureManager::doubleNames.size(); i++) {
    ...
    alloc_info[0].descriptorSetCount = TextureManager::doubleNames.size();
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::doubleNames.size());
...
void ShaderQueueSuit_CommonTexDouble::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/commonTexLand.vert";
    const char *fs = "./shaders/commonTexLand.frag";
...
void ShaderQueueSuit_CommonTexDouble::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_CommonTexDouble::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_CommonTexDouble::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_CommonTexDouble::ShaderQueueSuit_CommonTexDouble(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
