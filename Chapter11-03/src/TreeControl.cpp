...
void TreeControl::drawSelf(float bend_RIn, float direction_degreeIn) {//绘制树的方法
    for (int i = 0; i < trunkVector.size(); i++) {                    //遍历树干部件列表
        trunkVector[i]->drawSelf(VulkanDemoApp::cmdBuffer,            //绘制当前遍历到的树干部件
                                 VulkanDemoApp::sqsTrunk->pipelineLayout, VulkanDemoApp::sqsTrunk->pipeline,
                                 &(VulkanDemoApp::sqsTrunk->descSet[0]));
    }
    ...
    for (int i = 0; i < leafVector.size(); i++) {                                                                                                      //绘制树叶
        ...
        leafVector[i]->drawSelf(VulkanDemoApp::cmdBuffer, VulkanDemoApp::sqsBlend->pipelineLayout, VulkanDemoApp::sqsBlend->pipeline, &(VulkanDemoApp::sqsBlend->descSet[0]));
