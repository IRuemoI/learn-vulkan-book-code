...
void ShaderQueueSuit_Common::createShaderModule(VkDevice &device) {
    ...
    //编译顶点着色器：本节并没有使用书中提供的方法，书中的方法可以查看Chapter2.2的代码，可以看到由于shaderc库的大小很小，所以可执行程序要比不使用这种方法的体积小很多
    auto vtx_spv = compileGLSLtoSPIRV("./shaders/shader.vert", shaderc_glsl_default_vertex_shader);
    // 准备顶点着色器模块创建信息
    ...
    moduleCreateInfo.codeSize = vtx_spv.size();//顶点着色器SPV 数据总字节数
    moduleCreateInfo.pCode = vtx_spv.data();//顶点着色器SPV 数据
    ...
    auto frg_spv = compileGLSLtoSPIRV("./shaders/shader.frag", shaderc_glsl_default_fragment_shader);
    // 准备片元着色器模块创建信息
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;// 设置结构体类型
    moduleCreateInfo.pNext = nullptr;// 自定义数据的指针
    moduleCreateInfo.flags = 0;// 供将来使用的标志
    moduleCreateInfo.codeSize = frg_spv.size();//片元着色器SPV 数据总字节数
    moduleCreateInfo.pCode = frg_spv.data();//片元着色器SPV 数据
    // 创建片元着色器模块
...
void ShaderQueueSuit_Common::initVertexAttributeInfo() {
    ...
    vertexAttributes[0].binding = 0;// 第1个顶点输入属性的绑定点
    vertexAttributes[0].location = 0;// 第1个顶点输入属性的位置索引
    vertexAttributes[0].format = VK_FORMAT_R32G32B32_SFLOAT;// 第1个顶点输入属性的数据格式
    vertexAttributes[0].offset = 0;// 第1个顶点输入属性的偏移量

    vertexAttributes[1].binding = 0;// 第2个顶点输入属性的绑定点
    vertexAttributes[1].location = 1;// 第2个顶点输入属性的位置索引
    vertexAttributes[1].format = VK_FORMAT_R32G32B32_SFLOAT;// 第2个顶点输入属性的数据格式
    vertexAttributes[1].offset = 12;// 第2个顶点输入属性的偏移量
}

void ShaderQueueSuit_Common::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
    VkDynamicState dynamicStateEnables[VK_DYNAMIC_STATE_LINE_WIDTH];// 动态状态启用标志
...
void ShaderQueueSuit_Common::destroyPipeline(VkDevice &device) const {
    // 销毁管线
...
std::span<const uint32_t> ShaderQueueSuit_Common::compileGLSLtoSPIRV(const char *shaderFilePath, shaderc_shader_kind shader_kind) const {
    //编译顶点着色器，参数分别是着色器代码字符串，着色器类型，文件名
    auto shaderResult = compiler.CompileGlslToSpv(FileUtil::loadAssetStr(shaderFilePath), shader_kind, shaderFilePath);
    auto errorInfo = shaderResult.GetErrorMessage();
    //可以加判断，如果有错误信息(errorInfo!=""),就抛出异常
    std::cout << errorInfo;
    std::span<const uint32_t> shader_spv = {shaderResult.begin(), size_t(shaderResult.end() - shaderResult.begin()) * 4};
    return shader_spv;
}
