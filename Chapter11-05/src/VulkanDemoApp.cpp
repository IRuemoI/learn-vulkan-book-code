...
float VulkanDemoApp::rotation = 0;//旋转角度
float ratioT = 0;                 //旋转系数
float span = 0.003f;              //旋转步进
float operationT = 1;             //辅助增加位
float max_ration = 0.2f;          //最大范围
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 11.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texTri = new TexDrawableObject(TriangleData::vdata, TriangleData::dataByteCount, TriangleData::vCount, device, memoryProperties);
}
// 销毁绘制用物体
...
void VulkanDemoApp::drawObject() {
    ...
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1105_vulkan.bntex")]));
