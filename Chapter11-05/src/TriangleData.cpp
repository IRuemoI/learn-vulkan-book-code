...
void TriangleData::genVertexData() {        //顶点数据生成方法
    ...
    for (int i = 0; i < levelNum; i++)      //循环每一层生成小三角形
    {
        ...
        for (int j = 0; j < currBottomEdgeNum; j++) {
            ...
            //---顶点绘制方式
            alVertex.push_back(topX);
            alVertex.push_back(topY);
            alVertex.push_back(topZ);
            alVertex.push_back(leftBottomX);
            alVertex.push_back(leftBottomY);
            alVertex.push_back(leftBottomZ);
            alVertex.push_back(rightBottomX);
            alVertex.push_back(rightBottomY);
            alVertex.push_back(rightBottomZ);
            //-------纹理绘制方式
            alTexoor.push_back(topS);
            alTexoor.push_back(topT);
            alTexoor.push_back(leftBottomS);
            alTexoor.push_back(leftBottomT);
        ...
        for (int k = 0; k < currTopEdgeNum; k++) {              //循环产生当前层各个下三角形的顶点数据
            ...
            //逆时针卷绕-----
            alVertex.push_back(leftTopX);
            alVertex.push_back(leftTopY);
            alVertex.push_back(leftTopZ);
            alVertex.push_back(bottomX);
            alVertex.push_back(bottomY);
            alVertex.push_back(bottomZ);
            alVertex.push_back(rightTopX);
            alVertex.push_back(rightTopY);
            alVertex.push_back(rightTopZ);

            alTexoor.push_back(leftTopS);
            alTexoor.push_back(leftTopT);
            alTexoor.push_back(bottomS);
            alTexoor.push_back(bottomT);
            alTexoor.push_back(rightTopS);
