...
void VulkanDemoApp::createDrawableObject() {
    ...
    float *vdataIn = new float[48]{
            ...
            45, -5, 0, 1, 1, 1};
    //有效使用图元重启时的索引数据
    uint16_t *dataIndex = new uint16_t[9]{
            0, 1, 2, 3,
            0xFFFF,//特殊索引值
            4, 5, 6, 7};
    open = new DrawableObjectCommonLight(vdataIn, 48 * 4, 8, dataIndex, 9 * sizeof(uint16_t), 9, device, memoryProperties);//创建有效使用图元重启时的绘制物体

    //未有效使用图元重启时的索引数据
    dataIndex = new uint16_t[8]{
            0, 1, 2, 3,
            4, 5, 6, 7};
    close = new DrawableObjectCommonLight(vdataIn, 48 * 4, 8, dataIndex, 8 * sizeof(uint16_t), 8, device, memoryProperties);//创建未有效使用图元重启时的绘制物体
}
...
void VulkanDemoApp::flushUniformBuffer() {// 将当前帧相关数据送入一致变量缓冲
    ...
    VkResult result = vkMapMemory(device, sqsCRS->memUniformBuf, 0, sqsCRS->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCRS->bufferByteCount);
    vkUnmapMemory(device, sqsCRS->memUniformBuf);
}
...
void VulkanDemoApp::drawObject() {
    ...
    close->drawSelf(cmdBuffer, sqsCRS->pipelineLayout, sqsCRS->pipeline, &(sqsCRS->descSet[0]), 0, 8);//绘制正十边形
    MatrixState3D::popMatrix();                                                                       //恢复现场

    MatrixState3D::pushMatrix();                                                                     //保护现场
    MatrixState3D::translate(0, -10, 0);                                                             //沿Y 轴正方向平移50
    open->drawSelf(cmdBuffer, sqsCRS->pipelineLayout, sqsCRS->pipeline, &(sqsCRS->descSet[0]), 0, 9);//绘制正十边形
    MatrixState3D::popMatrix();                                                                      //恢复现场
...
void VulkanDemoApp::initPipeline() {
    sqsCRS = new ShaderQueueSuit_CommonReStart(&device, renderPass, memoryProperties);
}
