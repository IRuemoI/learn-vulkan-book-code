...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (preX < VulkanDemoApp::screenWidth / 4)//判断触控点x坐标是否小于允许的最小值
            VulkanDemoApp::vpCenterX = VulkanDemoApp::screenWidth / 4;
        if (preY > VulkanDemoApp::screenHeight / 4 * 3)//判断触控点y坐标是否大于允许的最大值
            VulkanDemoApp::vpCenterY = VulkanDemoApp::screenHeight / 4 * 3;
        if (preY < VulkanDemoApp::screenHeight / 4)//判断触控点y坐标是否小于允许的最小值
            VulkanDemoApp::vpCenterY = VulkanDemoApp::screenHeight / 4;

        printf("视口中心坐标:[X:%d,Y:%d]\n", VulkanDemoApp::vpCenterX, VulkanDemoApp::vpCenterX);
