...
public:
    ...
    ShaderQueueSuit_Common *sqsCL;// 着色器管线指针
    DrawableObjectCommonLight *triForDraw;// 绘制用三色三角形物体对象指针
    // 三角形旋转角度
    static float xAngle;//沿X轴的旋转角
    static float yAngle;//沿Y轴的旋转角
    static float zAngle;//沿Z轴的旋转角

    //视口中心坐标
    static int vpCenterX;//视口中心X坐标
    static int vpCenterY;//视口中心Y坐标
