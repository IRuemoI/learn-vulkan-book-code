...
int VulkanDemoApp::smallType = 0;
int VulkanDemoApp::bigType = 1;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {//创建绘制用物体的方法
    float *vdataIn = new float[30]{
            ...
            -2, -2, 0, 0, 1,
            2, -2, 0, 1, 1};
    texTri = new TexDrawableObject(vdataIn, 30 * 4, 6, device, memoryProperties);
    float *vdataIn1 = new float[30]{
            ...
            -7, -7, 0, 0, 1,
            7, -7, 0, 1, 1};
    texTri1 = new TexDrawableObject(vdataIn1, 30 * 4, 6, device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    //若 smallType 为 0（采用最近点采样）
    if (smallType == 0) {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0604_256Nearest.bntex")]));//绘制小纹理矩形
    }
    //若 smallType 为 1（采用线性采样）
    else {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0604_256Linear.bntex")]));//绘制小纹理矩形
    }
    MatrixState3D::popMatrix();         //恢复现场
    MatrixState3D::pushMatrix();        //保护现场
    MatrixState3D::translate(0, -5, 0); //沿 y 轴移动
    MatrixState3D::rotate(-30, 0, 0, 1);//绕 z 轴旋转
    //若 bigType 为 0（采用最近点采样）
    if (bigType == 0) {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0604_32Nearest.bntex")]));//绘制大纹理矩形
    }
    //若 bigType 为 1（采用线性采样）
    else {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0604_32Linear.bntex")]));//绘制大纹理矩形
    }
    MatrixState3D::popMatrix();//恢复现场
