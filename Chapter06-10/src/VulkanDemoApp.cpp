...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {//创建绘制用物体的方法
    ...
    texTri = new TexDrawableObject(vdataIn, 15 * 4, 3, device, memoryProperties);//创建三角形绘制物体
}
...
void VulkanDemoApp::drawObject() {
    ...
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0610_vulkan.bntexa")]), 0);//绘制左侧纹理三角形
    ...
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0610_vulkan.bntexa")]), 1);//绘制右侧纹理三角形
