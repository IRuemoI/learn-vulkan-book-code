...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.11", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    cubeForDraw = new Cube(device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    cubeForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制第一个立方体

    MatrixState3D::popMatrix();                                                                    //恢复现场
    MatrixState3D::pushMatrix();                                                                   //保护现场
    MatrixState3D::translate(80, 0, 0);                                                            //沿X 轴正方向平移80
    MatrixState3D::rotate(30, 0, 1, 0);                                                            //绕Y 轴旋转30°
    cubeForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//绘制第二个立方体
    MatrixState3D::popMatrix();                                                                    //恢复现场
