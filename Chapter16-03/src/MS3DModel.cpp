...
MS3DModel::MS3DModel(VkDevice &device,//构造函数
    ...
    initVertexBuffer(memoryProperties);                         //调用方法创建顶点数据缓冲
}
...
void MS3DModel::initVertexBuffer(VkPhysicalDeviceMemoryProperties &memoryProperties) {
    MS3DGroup *group;
    for (int i = 0; i < groups.size(); i++) {
        ...
        buf_info.flags = 0;
        VkResult result = vkCreateBuffer(*devicePointer, &buf_info, nullptr, &vertexDatabuf[i]);
        assert(result == VK_SUCCESS);
        vkGetBufferMemoryRequirements(*devicePointer, vertexDatabuf[i], &mem_reqs[i]);
        ...
        bool flag = memoryTypeFromProperties(memoryProperties, mem_reqs[i].memoryTypeBits, requirements_mask, &alloc_info.memoryTypeIndex);
        if (flag) {
            printf("确定内存类型成功 类型索引为%d\n", alloc_info.memoryTypeIndex);
        } else {
            printf("确定内存类型失败!\n");
        }
        result = vkAllocateMemory(*devicePointer, &alloc_info, nullptr, &vertexDataMem[i]);
        assert(result == VK_SUCCESS);
        uint8_t *pData;
        result = vkMapMemory(*devicePointer, vertexDataMem[i], 0, mem_reqs[i].size, 0, (void **) &pData);
        assert(result == VK_SUCCESS);
        memcpy(pData, vdata[i], dataByteCount);
        vkUnmapMemory(*devicePointer, vertexDataMem[i]);
        result = vkBindBufferMemory(*devicePointer, vertexDatabuf[i], vertexDataMem[i], 0);
        assert(result == VK_SUCCESS);
...
void MS3DModel::drawSelf(bool isUpdate, VkCommandBuffer &cmd,//绘制方法
    ...
    for (int i = 0; i < groups.size(); i++) {               //对模型中的所有组进行遍历
        ...
        if (group->getMaterialIndex() > -1) {               //若有材质（即需要贴纹理）
            ...
            desSetPointer = &VulkanDemoApp::sqsCL->descSet  //指定描述集（绑定纹理）
                                     [TextureManager::getVkDescriptorSetIndex(tp + texName2bntex(material->textureName))];
        }
        if (isUpdate) {                                  //若需要更新顶点数据缓冲
            ...
            vkUnmapMemory(VulkanDemoApp::device, vertexDataMem[i]);        //解除内存映射
        }
        vkCmdBindPipeline(cmd,//将当前使用的命令缓冲与指定管线绑定
                          VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
        vkCmdBindDescriptorSets(cmd,//将命令缓冲、管线布局、描述集绑定
                                VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1,
                                desSetPointer, 0, nullptr);
        const VkDeviceSize offsetsVertex[1] = {0};//顶点数据偏移量数组
        vkCmdBindVertexBuffers(cmd,               //将当前组顶点数据与当前使用的命令缓冲绑定
        ...
        vkCmdPushConstants(cmd,                           //将推送常量数据送入管线
                           pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float) * 16,
                           pushConstantData);
        vkCmdDraw(cmd, triangleCount * 3, 1, 0, 0);//绘制当前组
    }
}
MS3DModel::~MS3DModel() {
    delete jointHelper;
    ...
    for (int i = 0; iter4 != groups.end(); iter4++, i++) {
        vkDestroyBuffer(*devicePointer, vertexDatabuf[i], nullptr);
        vkFreeMemory(*devicePointer, vertexDataMem[i], nullptr);
    }
}
