...
int VulkanDemoApp::rectState = 0;
float VulkanDemoApp::moveSpan = 0.1;
bool VulkanDemoApp::moveFlag;
int VulkanDemoApp::stop = 0;
int VulkanDemoApp::up = 1;
int VulkanDemoApp::down = 2;
int VulkanDemoApp::left = 3;
int VulkanDemoApp::right = 4;

float VulkanDemoApp::rectX;
float VulkanDemoApp::rectY;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 8.1A", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texRect = new TexDrawableObject(RectData::vdata, RectData::dataByteCount, RectData::vCount, device, memoryProperties);
    CHForDraw = LoadUtil::loadFromFile("../models/0801_ch.obj", device, memoryProperties);
    CFTForDraw = LoadUtil::loadFromFile("../models/0801_cft.obj", device, memoryProperties);
    QTForDraw = LoadUtil::loadFromFile("../models/0801_qt.obj", device, memoryProperties);
    YHForDraw = LoadUtil::loadFromFile("../models/0801_yh.obj", device, memoryProperties);
    PMForDraw = LoadUtil::loadFromFile("../models/0801_pm.obj", device, memoryProperties);
    printf("模型加载成功  \n");
}
...
void VulkanDemoApp::drawObject() {
    ...
    texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0801A_lgq.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::popMatrix();
