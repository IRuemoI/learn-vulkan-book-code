...
void ShaderQueueSuit_Common::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    ...
    result = vkAllocateMemory(device, &alloc_info, nullptr,
                              &memUniformBuf);// 分配内存
...
void ShaderQueueSuit_Common::createPipelineLayout(VkDevice &device) {
    ...
    pPipelineLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;//指定结构体类型
    pPipelineLayoutCreateInfo.pNext = nullptr;                                      //自定义数据指针
...
void ShaderQueueSuit_Common::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
    ...
    for (int i = 0; i < 2; ++i) {                                                 //遍历数组中每个元素进行初始化
        ia[i].sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;//指定结构体类型
        ia[i].pNext = nullptr;                                                    //自定义数据的指针
    ...
    for (int i = 0; i < 2; i++) {
        ...
        result = vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineInfo, nullptr, &pipelines[i]);
