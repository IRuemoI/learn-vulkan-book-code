...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.8", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    beltForDraw = new DrawableObjectCommonLight(BeltData::vdata, BeltData::dataByteCount, BeltData::vCount, device, memoryProperties);
    CircleData::genVertexData();
    cirForDraw = new DrawableObjectCommonLight(CircleData::vdata, CircleData::dataByteCount, CircleData::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    beltForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipelines[0], &(sqsCL->descSet[0]));//绘制三角形条带
    MatrixState3D::popMatrix();                                                                        //恢复现场

    MatrixState3D::pushMatrix();                                                                      //保护现场
    MatrixState3D::translate(-90, 0, 0);                                                              //沿X轴负方向平移90
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipelines[1], &(sqsCL->descSet[0]));//绘制扇形
    MatrixState3D::popMatrix();                                                                       //恢复现场
