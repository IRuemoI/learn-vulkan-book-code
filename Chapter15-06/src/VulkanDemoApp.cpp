...
float VulkanDemoApp::isBloom = 0;
float VulkanDemoApp::rotateIndex;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.6", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1506_";
    base = LoadUtil::loadFromFile(mp + (string) ("base.obj"), device, memoryProperties);
    airship = LoadUtil::loadFromFile(mp + (string) ("airship.obj"), device, memoryProperties);
...
void VulkanDemoApp::flushTexToDesSetForToTex() {
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 3, sqsCTL->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::drawSceneToTex() {
    ...
    base->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                   &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) "base.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 50, 0);
    MatrixState3D::rotate(rotateIndex, 0, 1, 0);
    MatrixState3D::scale(0.06f, 0.06f, 0.06f);
    airship->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                      &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight(tp + (string) "airship.bntex")]));
    MatrixState3D::popMatrix();
