...
int VulkanDemoApp::pipeLineIndex = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 15.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    mountain = LoadUtil::loadFromFile("../models/1504_mountainPart.obj", device, memoryProperties);
    SkyBallData::genVertexData();
    skyForDraw = new TexDrawableObject(SkyBallData::vdata, SkyBallData::dataByteCount, SkyBallData::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    if (pipeLineIndex == 0) {
        ...
        MatrixState3D::translate(2.5f, 0.2f, 0);
        mountain->drawSelf(cmdBuffer, sqsDTL->pipelineLayout, sqsDTL->pipeline,
                           &(sqsDTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1504_mountain.bntex")]));
        ...
        mountain->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1504_mountain.bntex")]));
        MatrixState3D::popMatrix();
    }
    MatrixState3D::pushMatrix();
    skyForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCOmmonTex("../textures/1504_sky.bntex")]));
    MatrixState3D::popMatrix();
