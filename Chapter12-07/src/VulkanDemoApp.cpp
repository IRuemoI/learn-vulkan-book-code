...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 12.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::flushTexToDesSet() const// 更新绘制用描述集的方法
{
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsImage->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::drawObject() {
    ...
    MatrixState3D::translate(-3.3f, 3.3f, 0);
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1207_object.bntex")]));
    MatrixState3D::translate(6.6f, 0, 0);
    texTri->drawSelf(cmdBuffer, sqsImage->pipelineLayout, sqsImage->pipeline, &(sqsImage->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1207_object.bntex")]));
    MatrixState3D::translate(0, -6.6f, 0);
    texTri->drawSelf(cmdBuffer, sqsImage->pipelineLayout, sqsImage->pipeline, &(sqsImage->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1207_object1.bntex")]));
    MatrixState3D::translate(-6.6f, 0, 0);
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1207_object1.bntex")]));
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsImage = new ShaderQueueSuit_CommonTexImage(&device, renderPass, memoryProperties);
}
