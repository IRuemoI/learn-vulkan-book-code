...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 6.11", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createVulkanDevices() {
    ...
    vkGetPhysicalDeviceFeatures(gpus[USED_GPU_INDEX], &pdf);//获取物理设备属性
...
void VulkanDemoApp::createDrawableObject() {
    ...
    float *vdataIn = new float[30]{
            ...
            span, -span, 0, 1, 1,
            span, span, 0, 1, 0};
    texRect = new TexDrawableObject(vdataIn, 30 * 4, 6, device, memoryProperties);
}
...
void VulkanDemoApp::initMatrixAndLight() {
    MatrixState3D::setCamera(0, 0, 11, 0, 0, 0, 0, 1, 0);              // 初始化摄像机
...
void VulkanDemoApp::drawObject() {
    ...
    texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,                                                                 //绘制物体（正方形）
                      &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0611_mipmapIsotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
    ...
    MatrixState3D::rotate(90, 0, 1, 0);                                                                                                    //绕 y 轴旋转
    texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,                                                                   //绘制物体（正方形）
                      &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0611_mipmapAnisotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
    MatrixState3D::popMatrix();                                                                                                            //恢复现场
