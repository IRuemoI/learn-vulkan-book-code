...
std::vector<std::string> TextureManager::texNames = {"../textures/0611_mipmapIsotropy.bntex", "../textures/0611_mipmapAnisotropy.bntex"};
...
void TextureManager::initTextures(VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics) {
    ...
    for (int i = 0; i < texNames.size(); i++) {
        ...
        init_SPEC_Textures_ForMipMap(texNames[i], device, gpu, memoryProperties, cmdBuffer, queueGraphics, VK_FORMAT_R8G8B8A8_UNORM, ctdo, levels, i);
    }
}
void TextureManager::init_SPEC_Textures_ForMipMap(std::string texName, VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics, VkFormat format, TexDataObject *ctdo, int levels, int samplerIndex) {
