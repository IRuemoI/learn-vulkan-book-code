...
private:
    ...
    void createUniformBuffer(VkDevice &device,
                             VkPhysicalDeviceMemoryProperties &memoryProperties);// 创建一致变量缓冲
    ...
    void destroyPipeline(VkDevice &device);                                      // 销毁管线
public:
    ...
    ShaderQueueSuit_Common(VkDevice *deviceIn, VkRenderPass &renderPass,
                           VkPhysicalDeviceMemoryProperties &memoryProperties);// 构造函数
