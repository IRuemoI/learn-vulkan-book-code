...
#define OUT_TEX_COUNT 3//第一轮绘制时的输出数量
class VulkanDemoApp {
public:
    ...
    void createVulkanSelfColorBuffer();//创建绘制到纹理用颜色缓冲的方法
    ...
    void destroyVulkanSelfColorBuffer();//销毁绘制到纹理用颜色缓冲相关的方法
    void destroyFrameBuffer();// 销毁帧缓冲
    void destroyVulkanDepthBuffer();// 销毁深度缓冲相关
    void destroyVulkanSwapChain();// 销毁交换链
    void destroyVulkanCommandBuffer();// 销毁命令缓冲
    void destroyVulkanDevices();// 销毁逻辑设备
    void destroyVulkanInstance();// 销毁实例
    void createRenderPassScreen();
    void createRenderPassSelf();
    void createFrameBufferScreen();
    void createFrameBufferSelf();
    void destroyRenderPassSelf();
    void destroyRenderPassScreen();

    void drawSceneToTex();//将场景绘制到纹理的方法
    void drawSceneToScreen();//将纹理绘制到屏幕的方法
    //void flushUniformBufferForToTex();   //将场景绘制到纹理时使用的将一致变量数据送入缓冲的方法
    void flushTexToDesSetForToTex();//将场景绘制到纹理时使用的将纹理等数据与描述集关联的方法
    void flushUniformBufferForToScreen();//将纹理绘制到屏幕时使用的将一致变量数据送入缓冲的方法
    void flushTexToDesSetForToScreen();//将纹理绘制到屏幕时使用的将纹理等数据与描述集关联的方法
    void createVulkanSelfColorBufferSpec(VkFormat colorFormat, int index);
    ...
    VkClearValue clear_values_self[4];
