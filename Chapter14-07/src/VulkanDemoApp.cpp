...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.7", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
{
    VkImageView attachments[4];          //深度附件和颜色附件的图像视图数组
    attachments[5] = depthImageView;     //指定深度图像视图
    VkFramebufferCreateInfo fb_info = {};//构建帧缓冲创建信息结构体实例
    fb_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    fb_info.pNext = nullptr;
    fb_info.renderPass = renderPassSelf;//指定渲染通道
    fb_info.attachmentCount = 4;        //附件数量
...
void VulkanDemoApp::createDrawableObject() {
    treeForDraw = LoadUtil::loadFromFile("../models/1407_tree.obj", device, memoryProperties);
    CHForDraw = LoadUtil::loadFromFile("../models/1407_ch_t.obj", device, memoryProperties);
    carForDraw = LoadUtil::loadFromFile("../models/1407_car.obj", device, memoryProperties);
    pmForDraw = LoadUtil::loadFromFile("../models/1407_pm.obj", device, memoryProperties);
...
void VulkanDemoApp::flushTexToDesSetForToTex() {
    for (int i = 0; i < TextureManager::texNames.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 1, sqsCTL->writes, 0, nullptr);
    }
}
void VulkanDemoApp::flushUniformBufferForToScreen() {//将摄像机及光照数据送入一致变量缓冲的方法
...
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    ...
    vkUpdateDescriptorSets(device, 4, sqsD2D->writes, 0, nullptr);
}
void VulkanDemoApp::drawSceneToTex() {
    ...
    CHForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1407_ghxp.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-87, 0, 50);
    treeForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                          &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1407_tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87, 0, 50);
    carForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                         &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1407_car.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    pmForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1407_floor.bntex")]));
    MatrixState3D::popMatrix();
...
/*===========================================*/
void VulkanDemoApp::createVulkanSelfColorBufferSpec(VkFormat colorFormat, int index) {
    ...
    image_info.flags = 0;
    VkResult result = vkCreateImage(device, &image_info, nullptr, &(colorImage[index]));//创建图像
    assert(result == VK_SUCCESS);                                                       //检查是否创建成功
    ...
    result = vkCreateImageView(device, &view_info, nullptr, &(colorImageView[index]));//创建图像视图
    assert(result == VK_SUCCESS);                                                     //检查是否创建成功
}
void VulkanDemoApp::createVulkanSelfColorBuffer() {
    ...
    for (int i = 0; i < 3; i++) {                          //遍历所有颜色附件
        createVulkanSelfColorBufferSpec(colorFormat[i], i);//创建颜色附件对应图像
    }
}
