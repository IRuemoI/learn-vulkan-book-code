...
float VulkanDemoApp::xAngle;
float VulkanDemoApp::yAngle;

bool VulkanDemoApp::isDrawTask0ThreadShouldClose = false;
bool VulkanDemoApp::isDrawTask1ThreadShouldClose = false;
bool VulkanDemoApp::isDrawTask2ThreadShouldClose = false;
bool VulkanDemoApp::isDrawTask3ThreadShouldClose = false;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.8", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::mainLoop() {
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        //创建一个信号量，四个参数依次为 1、信号量特定属性 2、初始数量 3、最大数量 4、名称
        hSemaphore[i] = CreateSemaphore(nullptr, 1, 1, nullptr);//创建信号量
        WaitForSingleObject(hSemaphore[i], INFINITE);//申请信号量
    }
    ...
    drawTask1Thread = std::thread([this, capture0 = stta[1]] { drawTask1(capture0); });//创建独立绘制线程1
    drawTask1Thread.detach();//将线程1 独立出主线程

    stta[2] = new SCBArangeThreadTask(queueGraphicsFamilyIndex, &device, &(secondary_begin));
    drawTask2Thread = std::thread([this, capture0 = stta[2]] { drawTask2(capture0); });
    drawTask2Thread.detach();

    stta[3] = new SCBArangeThreadTask(queueGraphicsFamilyIndex, &device, &(secondary_begin));
    drawTask3Thread = std::thread([this, capture0 = stta[3]] { drawTask3(capture0); });
    drawTask3Thread.detach();
    ...
    isDrawTask0ThreadShouldClose = isDrawTask1ThreadShouldClose = isDrawTask2ThreadShouldClose = isDrawTask3ThreadShouldClose = true;
}
...
void VulkanDemoApp::createRenderPass() {
    ...
    secondary_begin.pNext = nullptr;//自定义数据指针
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1708_";
    objObject = LoadUtil::loadFromFile(mp + (string) ("plane01.obj"), device, memoryProperties);
    objObject1 = LoadUtil::loadFromFile(mp + (string) ("plane02.obj"), device, memoryProperties);
    objObject2 = LoadUtil::loadFromFile(mp + (string) ("plane03.obj"), device, memoryProperties);
    objObject3 = LoadUtil::loadFromFile(mp + (string) ("plane04.obj"), device, memoryProperties);
...
void VulkanDemoApp::drawSpec3D(SCBArangeThreadTask *stta, float pyx, float pyy, float pyz, float scx, float scy, float scz, ObjObject *oo, std::string texName, float *currMatrix, float *MVP) {
    const char *tp = "../textures/1708_";
    ...
    oo->drawSelf(stta->secondary_cmds[0], sqsCL->pipelineLayout, sqsCL->pipeline,//绘制物体(飞船)
                 &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(tp + (string) (texName))]), MVP, currMatrix);
}
void VulkanDemoApp::drawTask0(SCBArangeThreadTask *stta) {
    while (!isDrawTask0ThreadShouldClose) {
        WaitForSingleObject(hSemaphore[0], INFINITE);//申请信号量
        stta->initBeforeDraw();//绘制任务前准备二级命令缓冲
        planePosition0->update();//更新第0 组飞船的位置
        for (int i = 0; i < planePosition0->count; i++) {//循环绘制一组飞船
            drawSpec3D(stta, planePosition0->curPosition[i * 4 + 0], planePosition0->curPosition[i * 4 + 1], planePosition0->curPosition[i * 4 + 2], 0.2, 0.2, 0.2, objObject, "plane01.bntex", stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();//结束二级命令缓冲
        stta->doTaskFlag = true;//修改当前线程任务完成标志
    }
}
void VulkanDemoApp::drawTask1(SCBArangeThreadTask *stta) {
    while (!isDrawTask1ThreadShouldClose) {
        WaitForSingleObject(hSemaphore[1], INFINITE);
        stta->initBeforeDraw();
        planePosition1->update();
        for (int i = 0; i < planePosition1->count; i++) {
            drawSpec3D(stta, planePosition1->curPosition[i * 4 + 0], planePosition1->curPosition[i * 4 + 1], planePosition1->curPosition[i * 4 + 2], 0.2, 0.2, 0.2, objObject1, "plane02.bntex", stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
}
void VulkanDemoApp::drawTask2(SCBArangeThreadTask *stta) {
    while (!isDrawTask2ThreadShouldClose) {
        WaitForSingleObject(hSemaphore[2], INFINITE);
        stta->initBeforeDraw();
        planePosition2->update();
        for (int i = 0; i < planePosition2->count; i++) {
            drawSpec3D(stta, planePosition2->curPosition[i * 4 + 0], planePosition2->curPosition[i * 4 + 1], planePosition2->curPosition[i * 4 + 2], 0.2, 0.2, 0.2, objObject2, "plane03.bntex", stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
...
void VulkanDemoApp::drawTask3(SCBArangeThreadTask *stta) {
    while (!isDrawTask3ThreadShouldClose) {
        WaitForSingleObject(hSemaphore[3], INFINITE);
        stta->initBeforeDraw();
        planePosition3->update();
        for (int i = 0; i < planePosition3->count; i++) {
            drawSpec3D(stta, planePosition3->curPosition[i * 4 + 0], planePosition3->curPosition[i * 4 + 1], planePosition3->curPosition[i * 4 + 2], 0.2, 0.2, 0.2, objObject3, "plane04.bntex", stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
...
void VulkanDemoApp::drawObject() {
    ...
    cmd_buf_inheritance_info.framebuffer = framebuffers[currentBuffer];
    ...
    for (int i = 0; i < THREAD_COUNT; i++)//遍历四个独立绘制线程
    {
        ...
        ReleaseSemaphore(hSemaphore[i], 1, nullptr);//释放独立绘制线程完成一帧任务时所需的信号量
    }
    ...
    {
        vkCmdExecuteCommands(cmdBuffer, 1, stta[i]->secondary_cmds);
    }
