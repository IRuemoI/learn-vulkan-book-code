...
void ShaderQueueSuit_Common::initDescriptorSet(VkDevice &device) {
    ...
    type_count[0].descriptorCount = TextureManager::texNamesForObj.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::texNamesForObj.size();
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = nullptr;
    descriptor_pool.maxSets = TextureManager::texNamesForObj.size();
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNamesForObj.size();
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::texNamesForObj.size());
