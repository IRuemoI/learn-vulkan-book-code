...
#include <cassert>
#include <cstring>
TextureRect::TextureRect(float *vdataIn, int vdataByteCount, int vCountIn, int *indexDataIn, int indexDataByteCount, int indexCountIn, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void TextureRect::createVertexDataBuffer(int vdataByteCount, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    ...
    vkGetBufferMemoryRequirements(device, vertexDatabuf, &vdataMem_reqs);
    ...
    bool flag = memoryTypeFromProperties(memoryProperties, vdataMem_reqs.memoryTypeBits, requirements_mask, &alloc_info.memoryTypeIndex);
    assert(flag);
    printf("确定内存类型成功 类型索引为%d\n", alloc_info.memoryTypeIndex);
    result = vkAllocateMemory(device, &alloc_info, nullptr, &vertexDataMem);
    assert(result == VK_SUCCESS);
    uint8_t *pData;
    result = vkMapMemory(device, vertexDataMem, 0, vdataMem_reqs.size, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
...
void TextureRect::createVertexIndexDataBuffer(int indexDataByteCount, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    VkBufferCreateInfo buf_info = {};
    buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    buf_info.pNext = nullptr;
    buf_info.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
...
void TextureRect::drawSelf(VkCommandBuffer &cmd, VkPipelineLayout &pipelineLayout, VkPipeline &pipeline, VkDescriptorSet *desSetPointer) {
    uint8_t *pData;
    VkResult result = vkMapMemory(*devicePointer, vertexDataMem, 0, vdataMem_reqs.size, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vdata, vdataByteCount);
