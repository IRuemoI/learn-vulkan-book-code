...
#include <cassert>
void ShaderQueueSuit_Mirror::create_uniform_buffer_vert(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
/*创建片元着色器一致变量数据缓冲*/
void ShaderQueueSuit_Mirror::create_uniform_buffer_frag(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    ...
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    VkResult result = vkCreateBuffer(device, &buf_info, nullptr, &uniformBufFrag);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(device, uniformBufFrag, &mem_reqs);
    VkMemoryAllocateInfo alloc_info = {};
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.pNext = nullptr;
    alloc_info.memoryTypeIndex = 0;
    ...
    result = vkAllocateMemory(device, &alloc_info, nullptr, &memUniformBufFrag);
    assert(result == VK_SUCCESS);
    result = vkBindBufferMemory(device, uniformBufFrag, memUniformBufFrag, 0);
    assert(result == VK_SUCCESS);
...
void ShaderQueueSuit_Mirror::destroy_uniform_buffer_frag(VkDevice &device) {
    vkDestroyBuffer(device, uniformBufFrag, nullptr);
    vkFreeMemory(device, memUniformBufFrag, nullptr);
}
void ShaderQueueSuit_Mirror::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_Mirror::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_Mirror::initDescriptorSet(VkDevice &device) {
    ...
    descSet.resize(TextureManager::texNamesForWater.size() / 2);
...
void ShaderQueueSuit_Mirror::createShaderModule(VkDevice &device) {
    const char *ss = "./shaders/shadermirror.vert";
    const char *vs = "./shaders/shadermirror.frag";
    ...
    bool retVal = compileGLSLtoSPV(VK_SHADER_STAGE_VERTEX_BIT, FileUtil::loadAssetStr(ss).c_str(), vtx_spv);
    assert(retVal);
    ...
    retVal = compileGLSLtoSPV(VK_SHADER_STAGE_FRAGMENT_BIT, FileUtil::loadAssetStr(vs).c_str(), frag_spv);
    assert(retVal);
...
void ShaderQueueSuit_Mirror::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_Mirror::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_Mirror::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_Mirror::ShaderQueueSuit_Mirror(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
    this->devicePointer = deviceIn;
    create_uniform_buffer_vert(*devicePointer, memoryProperties);
    create_uniform_buffer_frag(*devicePointer, memoryProperties);
