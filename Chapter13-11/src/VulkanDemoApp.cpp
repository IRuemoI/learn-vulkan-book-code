...
bool VulkanDemoApp::isWaterThreadShouldClose = false;

float *VulkanDemoApp::mirrorMatrix;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.11", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initVulkan() {
    ...
    initMatrixAndLight();  // 初始化基本变换矩阵、摄像机矩阵、投影矩阵和光照
}

void VulkanDemoApp::mainLoop() {
    FPSUtil::init();// 初始化FPS计算
    waterThread = std::thread(&VulkanDemoApp::updateWater, this);
    waterThread.detach();
    ...
    for (uint32_t i = 0; i < queueFamilyCount; ++i)// 遍历设备对应的队列家族列表
    {
        ...
        if ((queueFamilyProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0)// 若此队列家族支持图形工作
        {
            ...
            if (pSupportsPresent[i] == VK_TRUE)// 如果当前队列家族支持显示工作
            {
                ...
                printf("队列家族索引=%d "
                       "同时支持Graphics（图形）和Present（显示）工作\n",
                       i);
                break;
            }
        }
...
void VulkanDemoApp::createVulkanSelfColorBuffer() {
    ...
    if (colorFormatProps.linearTilingFeatures & VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT) {
        ...
        printf("不支持VK_FORMAT_R8G8B8A8_UNORM！\n");
    }
    ...
    image_info.flags = 0;
    VkResult result = vkCreateImage(device, &image_info, nullptr, &colorImage);
    assert(result == VK_SUCCESS);
    ...
    vkGetImageMemoryRequirements(device, colorImage, &mem_reqs);
    ...
    assert(flag);
    printf("确定内存类型成功 类型索引为%d\n", mem_alloc.memoryTypeIndex);
    result = vkAllocateMemory(device, &mem_alloc, nullptr, &memColor);
    assert(result == VK_SUCCESS);
    result = vkBindImageMemory(device, colorImage, memColor, 0);
    assert(result == VK_SUCCESS);
...
void VulkanDemoApp::destroyVulkanSelfColorBuffer() {    //销毁颜色缓冲相关
    vkDestroyImageView(device, colorImageView, nullptr);//销毁图像视图对象
...
void VulkanDemoApp::createFrameBuffer() {
    ...
    VkResult result = vkCreateFramebuffer(device, &fb_info, nullptr, &selfTexFramebuffer);//创建服务于绘制到纹理的帧缓冲
...
void VulkanDemoApp::createDrawableObject() {
    houseObjForDraw = LoadUtil::loadFromFile("../models/1311_house.obj", device, memoryProperties);
    qiaoObjForDraw = LoadUtil::loadFromFile("../models/1311_qiao.obj", device, memoryProperties);
    tongObjForDraw = LoadUtil::loadFromFile("../models/1311_tong.obj", device, memoryProperties);
    treeObjForDraw = LoadUtil::loadFromFile("../models/1311_tree.obj", device, memoryProperties);
    skyboxObjForDraw = LoadUtil::loadFromFile("../models/1311_skybox.obj", device, memoryProperties);
    tableObjForDraw = LoadUtil::loadFromFile("../models/1311_table.obj", device, memoryProperties);
    woodObjForDraw = LoadUtil::loadFromFile("../models/1311_woodpile.obj", device, memoryProperties);
    mushRoomObjForDraw = LoadUtil::loadFromFile("../models/1311_mushroom.obj", device, memoryProperties);
    fbObjForDraw = LoadUtil::loadFromFile("../models/1311_fb.obj", device, memoryProperties);
    flowerObjForDraw = LoadUtil::loadFromFile("../models/1311_flower.obj", device, memoryProperties);
    VertData::genVertData();
    trWater = new TextureRect(VertData::vdata, VertData::dataByteCount, VertData::vCount, VertData::indexData, VertData::indexDataByteCount, VertData::indexCount, device, memoryProperties);
    printf("模型加载成功  \n");
}
...
void VulkanDemoApp::flushTexToDesSetForToTex() {
    for (int i = 0; i < TextureManager::texNamesForObj.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCL->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::flushUniformBufferForToScreen() {
    ...
    memcpy(pData, vertexUniformData, sqsCL->bufferByteCount);
    vkUnmapMemory(device, sqsCL->memUniformBuf);
    result = vkMapMemory(device, sqsM->memUniformBuf, 0, sqsM->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsM->bufferByteCount);
    vkUnmapMemory(device, sqsM->memUniformBuf);
    ...
    result = vkMapMemory(device, sqsM->memUniformBufFrag, 0, sqsM->bufferByteCountFrag, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsM->bufferByteCountFrag);
    vkUnmapMemory(device, sqsM->memUniformBufFrag);
}
void VulkanDemoApp::flushTexToDesSetForToScreen() {
    for (int i = 0; i < TextureManager::texNamesForWater.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 5, sqsM->writes, 0, nullptr);
    }
}
...
void VulkanDemoApp::updateWater() {
    while (!isWaterThreadShouldClose) {
        trWater->calVerticesNormalAndTangent();
        trWater->mytime += 0.01f;
        myLock.lock();
        trWater->copyData();
        myLock.unlock();
    }
...
void VulkanDemoApp::initPipeline() {
    sqsCL = new ShaderQueueSuit_Common(&device, renderPass, memoryProperties);
    sqsM = new ShaderQueueSuit_Mirror(&device, renderPass, memoryProperties);
}
...
void VulkanDemoApp::drawThings() {
    MatrixState3D::pushMatrix();
    skyboxObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_sky.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 0, -66.5f);
    houseObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_house.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-25, 5, -15);
    qiaoObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_qiao.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(15, -0.5f, 16);
    tongObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_stuff02.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(40, 0, -15);
    treeObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-25, -0.5f, 5);
    tableObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(23, -0.5f, 10);
    woodObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_stuff01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(46, 0, -43);
    mushRoomObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_vegetation01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-50, 0, -50);
    fbObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_stuff01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(30, 0, -43);
    flowerObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1311_vegetation01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
