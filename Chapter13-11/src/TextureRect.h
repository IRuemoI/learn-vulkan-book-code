...
public:
    ...
    float mytime = 0;    //计时器
    TextureRect(float *vdataIn, int dataByteCount, int vCountIn, int *indexDataIn, int indexDataByteCount, int indexCountIn, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties);
    ~TextureRect();                                                                                                                //析构函数
    void createVertexDataBuffer(int dataByteCount, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties);          //创建顶点数据缓冲的方法
