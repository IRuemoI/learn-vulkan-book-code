...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 16.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createRenderPass() {
    ...
    assert(result == VK_SUCCESS);
    clear_values[0].color.float32[0] = 0.8f;
    clear_values[0].color.float32[1] = 0.8f;
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1602_";
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("body.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("head.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("left_top.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("left_bottom.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("right_top.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("right_bottom.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("right_leg_top.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("right_leg_bottom.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("left_leg_top.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("left_leg_bottom.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("left_foot.obj"), device, memoryProperties));
    robotObj.push_back(LoadUtil::loadFromFile(mp + (string) ("right_foot.obj"), device, memoryProperties));
    robot = new Robot(robotObj);
    plane = LoadUtil::loadFromFile(mp + (string) ("floor.obj"), device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    robot->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1602_arm.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(0.5, 0.5, 0.5);
    plane->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1602_wenli.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initTextures() {
    TextureManager::initTextures(device, gpus[0], memoryProperties, cmdBuffer, queueGraphics);
}
