...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 12.5A", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::drawObject() {
    ...
    MatrixState3D::translate(3.3f, 0, 0);//处理后的图像
    texTri->drawSelf(cmdBuffer, sqsImage->pipelineLayout, sqsImage->pipeline, &(sqsImage->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1203_pm.bntex")]));
    MatrixState3D::translate(-6.6f, 0, 0);//正常图像
    texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1203_pm.bntex")]));
