...
float VulkanDemoApp::UNIT_SIZE = 10;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 13.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    objdraw = LoadUtil::loadFromFile("../models/1302_ball.obj", device, memoryProperties);
}
// 销毁绘制用物体
...
void VulkanDemoApp::flushUniformBuffer() {                                                                                 // 将当前帧相关数据送入一致变量缓冲
    float vertexUniformData[4] = {CameraUtil::camera9Para[0], CameraUtil::camera9Para[1], CameraUtil::camera9Para[2], 1.0};//摄像机位置XYZ
    uint8_t *pData;
