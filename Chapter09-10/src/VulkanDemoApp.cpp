...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 9.10", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::initMatrix() {
    ...
    LightManager::setLightPosition(0, 2.5f, 0);
...
void VulkanDemoApp::drawObject() {
    ...
    planeForDraw->drawSelf(cmdBuffer, sqsND->pipelineLayout, sqsND->pipeline,//绘制反射面(透明)
                           &(sqsND->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0909_mdbtm.bntex")]), false);
    MatrixState3D::popMatrix();                                             //恢复现场
    MatrixState3D::pushMatrix();                                            //保护现场
    MatrixState3D::translate(0, bfc->currentY + 0.4, 0);                    //执行平移
