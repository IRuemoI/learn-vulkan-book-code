...
float VulkanDemoApp::lightElevation = 40;
int VulkanDemoApp::lightAzimuth = 180;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.5", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObjectCH1 = LoadUtil::loadFromFileNoNormal("../models/1405_ch1.obj", device, memoryProperties);
    objObjectCH2 = LoadUtil::loadFromFileNoNormal("../models/1405_ch2.obj", device, memoryProperties);
    objObjectPTCH = LoadUtil::loadFromFile("../models/1405_ptch.obj", device, memoryProperties);
    objObjectPM = LoadUtil::loadFromFileNoNormal("../models/1405_pm.obj", device, memoryProperties);
    printf("模型加载成功\n");
}
    ...
    float vertexUniformDataShadow[32] =
            {
                    ...
                    MProjCameraMatrix[8],
                    MProjCameraMatrix[9],
                    MProjCameraMatrix[10],
                    MProjCameraMatrix[11],
                    MProjCameraMatrix[12],
                    MProjCameraMatrix[13],
                    MProjCameraMatrix[14],
                    MProjCameraMatrix[15],
            };
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsC->memUniformBuf, 0, sqsC->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformDataShadow, sqsC->bufferByteCount);
    vkUnmapMemory(device, sqsC->memUniformBuf);
...
void VulkanDemoApp::flushTexToDesSet() {
    ...
    vkUpdateDescriptorSets(device, 2, sqsC->writes, 0, nullptr);
...
void VulkanDemoApp::drawObject() {
    ...
    objObjectPM->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                                &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1405_pm.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    objObjectCH1->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                                 &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1405_c1.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    objObjectCH2->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                                 &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1405_c2.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    ...
    objObjectPTCH->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline,
                            &(sqsC->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1405_ghxp.bntex")]), 1);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(1.2f, 1.2f, 1.2f);
    MatrixState3D::translate(18, 0, 20);
    objObjectPTCH->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline,
                            &(sqsC->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/1405_ghxp.bntex")]), 0);
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsC = new ShaderQueueSuit_Shadow(&device, renderPass, memoryProperties);
}
