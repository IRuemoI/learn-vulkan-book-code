...
void ObjObject::drawSelf(VkCommandBuffer &cmd, VkPipelineLayout &pipelineLayout, VkPipeline &pipeline, VkDescriptorSet *desSetPointer, float bfds0, float bfds1, float bfds2, float nfds, float needsNormal) {
    ...
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_GEOMETRY_BIT, 0, sizeof(float) * 20, pushConstantData);
    delete[] needsNormalData;
    memcpy(pushConstantData, mm, sizeof(float) * 16);
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, sizeof(float) * 20, sizeof(float) * 16, pushConstantData);
    float *tesFactor = new float[4]{bfds0, bfds1, bfds2, nfds};
    memcpy(pushConstantData, tesFactor, sizeof(float) * 4);
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, sizeof(float) * 36, sizeof(float) * 4, pushConstantData);
    delete[] tesFactor;
