...
void ShaderQueueSuit_Common::destroyUniformBuffer(VkDevice &device) {
...
void ShaderQueueSuit_Common::createShaderModule(VkDevice &device) {
    ...
    std::string tcsStr = FileUtil::loadAssetStr("./shaders/commonTexLight.tcs");
    std::string tesStr = FileUtil::loadAssetStr("./shaders/commonTexLight.tes");
    std::string geoStr = FileUtil::loadAssetStr("./shaders/commonTexLight.geo");
    ...
    retVal = compileGLSLtoSPV(VK_SHADER_STAGE_GEOMETRY_BIT, geoStr.c_str(), geo_spv);
    assert(retVal);
    printf("几何着色器脚本编译SPV成功！");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    moduleCreateInfo.pNext = nullptr;
    moduleCreateInfo.flags = 0;
    moduleCreateInfo.codeSize = geo_spv.size() * sizeof(unsigned int);
    moduleCreateInfo.pCode = geo_spv.data();
    result = vkCreateShaderModule(device, &moduleCreateInfo, nullptr, &shaderStages[3].module);
    assert(result == VK_SUCCESS);
    ...
    result = vkCreateShaderModule(device, &moduleCreateInfo, nullptr, &shaderStages[4].module);
    assert(result == VK_SUCCESS);
}
void ShaderQueueSuit_Common::destroyShaderModule(VkDevice &device) {
    ...
    vkDestroyShaderModule(device, shaderStages[4].module, nullptr);
}
...
void ShaderQueueSuit_Common::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
    ...
    pipelineInfo.stageCount = 5;//管线的着色阶段数量
