...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 7.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObject = LoadUtil::loadFromFile("../models/0704_ch_t.obj", device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0704_ghxp.bntex")]));
...
void VulkanDemoApp::initTexture() {
