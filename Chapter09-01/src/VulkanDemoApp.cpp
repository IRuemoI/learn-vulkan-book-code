...
TexDrawableObject *VulkanDemoApp::trees[9];
float VulkanDemoApp::cx = 0;
float VulkanDemoApp::cz = 60;
float VulkanDemoApp::direction = 0;
float VulkanDemoApp::Offset = 60;
float VulkanDemoApp::DEGREE_SPAN = (float) (3.0 / 180.0f * 3.14);
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 9.1", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createRenderPass() {
    ...
    clear_values[0].color.float32[0] = 1.0f;  // 帧缓冲清除用R分量值
    clear_values[0].color.float32[1] = 1.0f;  // 帧缓冲清除用G分量值
    clear_values[0].color.float32[2] = 1.0f;  // 帧缓冲清除用B分量值
    clear_values[0].color.float32[3] = 1.0f;  // 帧缓冲清除用A分量值
...
void VulkanDemoApp::createDrawableObject() {
    ...
    texRect = new TexDrawableObject(RectData::vData, RectData::dataByteCount,
                                    RectData::vCount, device, memoryProperties, 0, 0);//创建绘制底部沙漠用的纹理矩形
    ...
    for (int i = 0; i < 9; i++) {//遍历植物组
        trees[i] = new TexDrawableObject(TreeData::vData, TreeData::dataByteCount, TreeData::vCount, device, memoryProperties, position[i][0], position[i][2]);
    }
...
void VulkanDemoApp::drawObject() {
    ...
    texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0901_desert.bntex")]));
    MatrixState3D::popMatrix();
    for (int i = 0; i < 9; i++) {
        MatrixState3D::pushMatrix();
        trees[i]->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0901_tree.bntex")]));
        MatrixState3D::popMatrix();
    }
