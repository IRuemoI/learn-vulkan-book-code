...
#include <assert.h>
ObjObjectQiu::ObjObjectQiu(float *vdataIn, int dataByteCount, int vCountIn, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ObjObjectQiu::drawSelf(VkCommandBuffer &cmd, VkPipelineLayout &pipelineLayout, VkPipeline &pipeline, VkDescriptorSet *desSetPointer) {
    ...
    vkCmdDraw(cmd, vCount, PlanetConstant::paiNumber, 0, 0);//采用实例渲染绘制海量小星体
}