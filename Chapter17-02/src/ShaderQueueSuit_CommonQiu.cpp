...
void ShaderQueueSuit_CommonQiu::createUniformBuffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {
...
void ShaderQueueSuit_CommonQiu::destroyUniformBuffer(VkDevice &device) {
    vkDestroyBuffer(device, uniformBuf, nullptr);
    vkFreeMemory(device, memUniformBuf, nullptr);
}
void ShaderQueueSuit_CommonQiu::createPipelineLayout(VkDevice &device) {
...
void ShaderQueueSuit_CommonQiu::destroyPipelineLayout(VkDevice &device) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) {
        vkDestroyDescriptorSetLayout(device, descLayouts[i], nullptr);
    }
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);
}
void ShaderQueueSuit_CommonQiu::initDescriptorSet(VkDevice &device) {
    VkDescriptorPoolSize type_count[3];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::doubleStone.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::doubleStone.size();
    type_count[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[2].descriptorCount = TextureManager::doubleStone.size();

    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = nullptr;
    descriptor_pool.maxSets = TextureManager::doubleStone.size();
    ...
    for (int i = 0; i < TextureManager::doubleStone.size(); i++) {
    ...
    alloc_info[0].descriptorSetCount = TextureManager::doubleStone.size();
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::doubleStone.size());
...
void ShaderQueueSuit_CommonQiu::createShaderModule(VkDevice &device) {
    const char *vs = "./shaders/commonTexQiu.vert";
    const char *fs = "./shaders/commonTexQiu.frag";
...
void ShaderQueueSuit_CommonQiu::destroyShaderModule(VkDevice &device) {
...
void ShaderQueueSuit_CommonQiu::createPipeline(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_CommonQiu::destroyPipeline(VkDevice &device) {
    vkDestroyPipeline(device, pipeline, nullptr);
    vkDestroyPipelineCache(device, pipelineCache, nullptr);
}
ShaderQueueSuit_CommonQiu::ShaderQueueSuit_CommonQiu(VkDevice *deviceIn, VkRenderPass &renderPass, VkPhysicalDeviceMemoryProperties &memoryProperties) {
