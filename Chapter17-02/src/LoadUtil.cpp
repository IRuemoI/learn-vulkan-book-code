...
ObjObjectQiu *LoadUtil::loadFromFileQiu(const string &vname, VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryProperties) {//读取obj 文件内容生成绘制用物体对象的方法
    ObjObjectQiu *lo;//指向生成的绘制用物体对象的指针
    vector<float> alv;
    vector<float> alvResult;
    vector<float> alt;
    vector<float> altResult;
    ...
    lo = new ObjObjectQiu(vdataIn, dataByteCount, vCount, device, memoryProperties);//创建绘制用物体对象
    return lo;//返回指向绘制用物体对象的指针
}
