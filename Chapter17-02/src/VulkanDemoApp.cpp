...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    const char *mp = "../models/1702_";
    objObject = LoadUtil::loadFromFile(mp + (string) ("qiu.obj"), device, memoryProperties);
    objQiu = LoadUtil::loadFromFileQiu(mp + (string) ("stone.obj"), device, memoryProperties);
    objQiuTwo = LoadUtil::loadFromFileQiu(mp + (string) ("stone2.obj"), device, memoryProperties);
    objQiuThree = LoadUtil::loadFromFileQiu(mp + (string) ("stone3.obj"), device, memoryProperties);
}
...
void VulkanDemoApp::initMatrix() {
    ...
    MatrixState3D::setCamera(-20, -40, 80, 0, 0, 0, 0, -1, 0);
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    result = vkMapMemory(device, sqsQiu->memUniformBuf, 0, sqsQiu->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, qiuUniformData, sqsQiu->bufferByteCount);
    vkUnmapMemory(device, sqsQiu->memUniformBuf);

    qiuUniformData[24] = PlanetConstant::distanceDaiBig;

    result = vkMapMemory(device, sqsQiuTwo->memUniformBuf, 0, sqsQiuTwo->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, qiuUniformData, sqsQiuTwo->bufferByteCount);
    vkUnmapMemory(device, sqsQiuTwo->memUniformBuf);
}
void VulkanDemoApp::flushTexToDesSet() {
    ...
    for (int i = 0; i < TextureManager::doubleStone.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 3, sqsQiu->writes, 0, nullptr);
        ...
        vkUpdateDescriptorSets(device, 3, sqsQiuTwo->writes, 0, nullptr);
    }
}
void VulkanDemoApp::drawObject() {
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1702_qiu.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    objQiu->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline, &(sqsQiu->descSet[0]));
    objQiuTwo->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline, &(sqsQiu->descSet[1]));
    objQiuThree->drawSelf(cmdBuffer, sqsQiu->pipelineLayout, sqsQiu->pipeline, &(sqsQiu->descSet[2]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    objQiu->drawSelf(cmdBuffer, sqsQiuTwo->pipelineLayout, sqsQiuTwo->pipeline, &(sqsQiuTwo->descSet[0]));
    objQiuTwo->drawSelf(cmdBuffer, sqsQiuTwo->pipelineLayout, sqsQiuTwo->pipeline, &(sqsQiuTwo->descSet[1]));
    objQiuThree->drawSelf(cmdBuffer, sqsQiuTwo->pipelineLayout, sqsQiuTwo->pipeline, &(sqsQiuTwo->descSet[2]));
    MatrixState3D::popMatrix();
...
void VulkanDemoApp::initPipeline() {
    ...
    sqsQiu = new ShaderQueueSuit_CommonQiu(&device, renderPass, memoryProperties);
    sqsQiuTwo = new ShaderQueueSuit_CommonQiu(&device, renderPass, memoryProperties);
}
