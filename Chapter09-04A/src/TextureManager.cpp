...
void TextureManager::init_SPEC_Textures_ForMipMap(std::string texName, VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics, VkFormat format, CommonTexDataObject *ctdo, int levels) {
...
void TextureManager::initTextures(VkDevice &device, VkPhysicalDevice &gpu, VkPhysicalDeviceMemoryProperties &memoryProperties, VkCommandBuffer &cmdBuffer, VkQueue &queueGraphics) {
    ...
    for (int i = 0; i < texNames.size(); i++) {
        ...
        printf("%s w %d h %d L %d\n", texNames[i].c_str(), ctdo->width, ctdo->height, levels);
        init_SPEC_Textures_ForMipMap(texNames[i], device, gpu, memoryProperties,
