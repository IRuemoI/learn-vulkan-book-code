...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 8.2", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    ...
    ballForDraw = new TexLightObject(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryProperties);
    BallData::genBallData(3, 30.2);
    cloudBallForDraw = new TexLightObject(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryProperties);
    SkyData::genSkyData(400);
    skyForDrawBig = new ColorObject(SkyData::vdata, SkyData::dataByteCount, SkyData::vCount, device, memoryProperties, 4.0f);
    SkyData::genSkyData(1600);
    skyForDrawSmall = new ColorObject(SkyData::vdata, SkyData::dataByteCount, SkyData::vCount, device, memoryProperties, 2.0f);
}
...
void VulkanDemoApp::flushUniformBuffer() const {//将当前帧的一致变量数据送入一致变量缓冲的方法
    ...
    result = vkMapMemory(device, sqsDTL->memUniformBuf, 0, sqsDTL->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsDTL->bufferByteCount);
    vkUnmapMemory(device, sqsDTL->memUniformBuf);
    result = vkMapMemory(device, sqsCloud->memUniformBuf, 0, sqsCloud->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCloud->bufferByteCount);
    vkUnmapMemory(device, sqsCloud->memUniformBuf);
}

void VulkanDemoApp::flushTexToDesSet() const {
    ...
    for (int i = 0; i < TextureManager::texNamesPair.size() / 2; i++) {
        ...
        vkUpdateDescriptorSets(device, 3, sqsDTL->writes, 0, nullptr);
    }
    for (int i = 0; i < TextureManager::texNameCloud.size(); i++) {
        ...
        vkUpdateDescriptorSets(device, 2, sqsCloud->writes, 0, nullptr);
    }
}

void VulkanDemoApp::drawObject() {
    eAngle = float(eAngle + 0.4);
    if (eAngle >= 360) eAngle = 0;//更新地球自转角
    mAngle = float(mAngle + 0.4);
    if (mAngle >= 360) mAngle = 0;//更新月球自转角
    sAngle = float(sAngle + 0.02);
    if (sAngle >= 360) sAngle = 0;    //更新星空自转角
    ...
    ballForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline,//绘制月球
                          &(sqsSTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("../textures/0606_moon.bntex")]));
    MatrixState3D::popMatrix();                                                //恢复现场
    MatrixState3D::pushMatrix();                                               //保护现场
    ...
    MatrixState3D::popMatrix();                                                //恢复现场                                                                                                                                                         //恢复现场
                                                                               // 恢复现场
...
void VulkanDemoApp::initTextures()//初始化纹理的方法
{
    TextureManager::initTextures(device, gpus[USED_GPU_INDEX], memoryProperties, cmdBuffer, queueGraphics);
}
void VulkanDemoApp::destroyTextures()//销毁纹理的方法
{
    TextureManager::destroyTextures(device);
}

void VulkanDemoApp::initPipeline() {
    sqsC = new ShaderQueueSuit_Color(&device, renderPass, memoryProperties);
    sqsDTL = new ShaderQueueSuit_DoubleTexLight(&device, renderPass, memoryProperties);
    sqsSTL = new ShaderQueueSuit_SingleTexLight(&device, renderPass, memoryProperties);
    sqsCloud = new ShaderQueueSuit_Cloud(&device, renderPass, memoryProperties);
}
