...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 17.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    const char *tModel = "../models/1704_";
    LYobjPeople = LoadUtil::loadFromFile(tModel + (string) "people.obj", device, memoryProperties);
    //FlatData::genVertexData();
    LYobjDiban = LoadUtil::loadFromFile(tModel + (string) "pm.obj", device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
            ...
            if (jrow != centerNum || kcol != centerNum) {
                ...
                if (isSZTJC)//开启视锥体剪裁
                {
                    if (SZTJCUtil::isAABBInSZT(MatrixState3D::getGLFinalMatrix(), LYobjPeople->aabb)) {
                        LYobjPeople->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1704_people.bntex")]));
                        currDrawCount++;
                    }
                } else {
                    LYobjPeople->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1704_people.bntex")]));
    ...
    if (isSZTJC)//开启视锥体剪裁
    {
        if (SZTJCUtil::isAABBInSZT(MatrixState3D::getGLFinalMatrix(), LYobjDiban->aabb)) {
            LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1704_diban.bntex")]));
            currDrawCount++;
        }
    } else {
        LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1704_diban.bntex")]));
