...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 5.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    BallData::genBallData(9);                                                                                                         //生成球的顶点数据和法向量数据
    ballForDraw = new DrawableObjectCommonLight(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryProperties);//创建绘制用球
}
