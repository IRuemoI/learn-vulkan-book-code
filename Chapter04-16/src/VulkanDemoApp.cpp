...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.16", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::drawObject() {
    ...
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0);//绘制整正十边形

    MatrixState3D::popMatrix();                                                                                                         //恢复现场
    MatrixState3D::pushMatrix();                                                                                                        //保护现场
    MatrixState3D::translate(0, -50, 0);                                                                                                //沿Y 轴负方向平移50
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), sizeof(VkDrawIndexedIndirectCommand));//绘制正十边形的下半部分
    MatrixState3D::popMatrix();                                                                                                         //恢复现场
