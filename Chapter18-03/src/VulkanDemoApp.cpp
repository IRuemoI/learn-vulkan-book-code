...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 18.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createVulkanDevices() {
    ...
    vkGetPhysicalDeviceProperties(gpus[USED_GPU_INDEX], &pdp);//获取各项属性值
    printf("framebufferColorSampleCounts:%d framebufferDepthSampleCounts:%d",//打印最大多重采样数
           pdp.limits.framebufferColorSampleCounts, pdp.limits.framebufferDepthSampleCounts);
    assert(pdp.limits.framebufferColorSampleCounts >=
                   MULSAMPLE_COUNT &&
           pdp.limits.framebufferDepthSampleCounts >= MULSAMPLE_COUNT);//判断当前设备支持的最大多重采样数是否满足需要
}
...
void VulkanDemoApp::createVulkanDepthBuffer() {
    ...
    result = vkCreateImage(device, &image_info, nullptr, &depthImageMul);
    assert(result == VK_SUCCESS);
    vkGetImageMemoryRequirements(device, depthImageMul, &mem_reqs);
    mem_alloc.allocationSize = mem_reqs.size;
    requirements_mask = 0;
    flag = memoryTypeFromProperties(memoryProperties, mem_reqs.memoryTypeBits, requirements_mask, &mem_alloc.memoryTypeIndex);
    assert(flag);
    printf("确定内存类型成功 类型索引为%d\n", mem_alloc.memoryTypeIndex);
    result = vkAllocateMemory(device, &mem_alloc, nullptr, &memDepthMul);
    assert(result == VK_SUCCESS);
    result = vkBindImageMemory(device, depthImageMul, memDepthMul, 0);
    assert(result == VK_SUCCESS);
    view_info.image = depthImageMul;
    result = vkCreateImageView(device, &view_info, nullptr, &depthImageViewMul);
    assert(result == VK_SUCCESS);
}
void VulkanDemoApp::destroyVulkanDepthBuffer() {
    ...
    vkDestroyImageView(device, depthImageViewMul, nullptr);
    vkDestroyImage(device, depthImageMul, nullptr);
    vkFreeMemory(device, memDepthMul, nullptr);
    printf("销毁深度缓冲相关成功!\n");
}
...
void VulkanDemoApp::createDrawableObject() {
    objObject = LoadUtil::loadFromFile("../models/1803_SpaceStation.obj", device, memoryProperties);
}
...
void VulkanDemoApp::drawObject() {
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1803_kjz.bntex")]));
