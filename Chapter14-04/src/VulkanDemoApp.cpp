...
float VulkanDemoApp::ty = 0;
float VulkanDemoApp::tz = 0;
...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 14.4", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    objObjectCH = LoadUtil::loadFromFile("../models/1404_ver.obj", device, memoryProperties);
    objObjectPM = LoadUtil::loadFromFile("../models/1404_pm.obj", device, memoryProperties);
    printf("模型加载成功\n");
}
...
void VulkanDemoApp::flushUniformBuffer() {
    ...
    uint8_t *pData;
    VkResult result = vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, VertUniformData, sqsCT->bufferByteCount);
    vkUnmapMemory(device, sqsCT->memUniformBuf);
    result = vkMapMemory(device, sqsCL->memUniformBuf, 0, sqsCL->bufferByteCount, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    ...
    result = vkMapMemory(device, sqsCT->memUniformBufFrag, 0, sqsCT->bufferByteCountFrag, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsCT->bufferByteCountFrag);
    vkUnmapMemory(device, sqsCT->memUniformBufFrag);
    result = vkMapMemory(device, sqsCL->memUniformBufFrag, 0, sqsCL->bufferByteCountFrag, 0, (void **) &pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsCL->bufferByteCountFrag);
    vkUnmapMemory(device, sqsCL->memUniformBufFrag);
}
void VulkanDemoApp::flushTexToDesSet() {
    ...
    vkUpdateDescriptorSets(device, 3, sqsCT->writes, 0, nullptr);
    ...
    vkUpdateDescriptorSets(device, 3, sqsCL->writes, 0, nullptr);
}

void VulkanDemoApp::drawObject() {
    FPSUtil::calFPS();
    FPSUtil::before();
    LightManager::getTYJZ(screenWidth * 1.0f / screenHeight);
    ...
    objObjectCH->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0);
    objObjectCH->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 1);
...
void VulkanDemoApp::initPipeline() {
    sqsCT = new ShaderQueueSuit_Common(&device, renderPass, memoryProperties);
    sqsCL = new ShaderQueueSuit_CommonLight(&device, renderPass, memoryProperties);
}
