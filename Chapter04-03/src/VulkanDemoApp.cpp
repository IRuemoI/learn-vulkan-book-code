...
void VulkanDemoApp::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan 4.3", nullptr, nullptr);// 创建窗口并返回窗口的句柄
...
void VulkanDemoApp::createDrawableObject() {
    SixPointedStar::genStarData(0.2, 0.5, 0);// 生成六角星顶点数据和颜色数据
    objForDraw = new DrawableObjectCommonLight(SixPointedStar::vdata, SixPointedStar::dataByteCount, SixPointedStar::vCount, device, memoryProperties);
}
...
void VulkanDemoApp::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio * 0.4f, ratio * 0.4f, -1 * 0.4f, 1 * 0.4f, 1.0f, 20);//设置正交投影参数
}
...
void VulkanDemoApp::drawObject() {
    ...
    for (int i = 0; i <= 5; i++) {                                                                    //循环绘制所有六角星
        ...
        objForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));//恢复现场
